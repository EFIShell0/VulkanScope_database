...
asset=root/'assets/app.v1006.js'
app=asset.read_text(encoding='utf-8') if asset.is_file() else ''
if not a.skip_version:
    if 'VulkanScope Database <strong>1.0.6</strong>' not in index: errors.append('Database 0.39.25 footer missing')
    if pkg.get('version')!='1.0.6': errors.append('Worker package version must be 1.0.1')
if './assets/app.v1006.js' not in index or './config.js?v=1006' not in index: errors.append('0.39.22 cache-busted frontend references missing')
...
print('PASS Database 1.0.6 retained loading/scroll/floor/device-type contract')
