...
contract_path = root/'regression/1.4.6_to_1.4.7_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.4.7.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.4.7-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_4_7_floor_metadata_coherence.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_4_7_floor_metadata_coherence_negative_mutations.py')],
        cwd=extracted, check=True
    )
