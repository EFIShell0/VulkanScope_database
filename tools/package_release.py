...
contract_path = root/'regression/1.4.5_to_1.4.6_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.4.6.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.4.6-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_4_6_submission_floor.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_4_6_submission_floor_negative_mutations.py')],
        cwd=extracted, check=True
    )
