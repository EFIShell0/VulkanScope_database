...
contract_path = root/'regression/1.3.8_to_1.3.9_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.3.9.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.3.9-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_3_9_regional_filter_layout_profile.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_3_9_regional_filter_layout_profile_negative_mutations.py')],
        cwd=extracted, check=True
    )
