...
contract_path = root/'regression/1.2.15_to_1.3.0_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.3.0.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.3.0-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_3_0_trademark_licenses_full_audit.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_3_0_trademark_licenses_full_audit_negative_mutations.py')],
        cwd=extracted, check=True
    )
