...
contract_path = root/'regression/1.0.9_to_1.0.10_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.10.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.10-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_10_reports_render_hotfix.py')],
        cwd=extracted, check=True
    )
