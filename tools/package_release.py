...
contract_path = root/'regression/1.2.5_to_1.2.6_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.6.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.6-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_6_cross_browser_detail.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_6_cross_browser_detail_negative_mutations.py')],
        cwd=extracted, check=True
    )
