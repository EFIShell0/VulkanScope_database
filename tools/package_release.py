...
contract_path = root/'regression/1.0.17_to_1.0.18_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.18.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.18-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_18_full_audit.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_18_full_audit_negative_mutations.py')],
        cwd=extracted, check=True
    )
