...
contract_path = root/'regression/1.3.0_to_1.3.1_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.3.1.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.3.1-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_3_1_freshness_filters_license_live_sync.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_3_1_freshness_filters_license_live_sync_negative_mutations.py')],
        cwd=extracted, check=True
    )
