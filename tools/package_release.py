...
contract_path = root/'regression/1.0.25_to_1.2.0_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.0.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.0-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_0_workspace_floor_settings.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_0_workspace_floor_settings_negative_mutations.py')],
        cwd=extracted, check=True
    )
