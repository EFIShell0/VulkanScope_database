...
contract_path = root/'regression/1.0.13_to_1.0.14_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.14.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.14-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_14_database_ui_integrity.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_14_database_ui_integrity_negative_mutations.py')],
        cwd=extracted, check=True
    )
