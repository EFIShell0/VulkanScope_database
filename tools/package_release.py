...
contract_path = root/'regression/1.3.10_to_1.4.0_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.4.0.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.4.0-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_4_0_submission_settings_ip_filter_boot.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_4_0_submission_settings_ip_filter_boot_negative_mutations.py')],
        cwd=extracted, check=True
    )
