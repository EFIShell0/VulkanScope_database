...
contract_path = root/'regression/1.0.15_to_1.0.16_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.16.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.16-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_16_live_modal_versions.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_16_live_modal_versions_negative_mutations.py')],
        cwd=extracted, check=True
    )
