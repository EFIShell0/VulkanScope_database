...
contract_path = root/'regression/1.2.10_to_1.2.11_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.11.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.11-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_11_compare_mobile_identity.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_11_compare_mobile_identity_negative_mutations.py')],
        cwd=extracted, check=True
    )
