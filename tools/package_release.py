...
contract_path = root/'regression/1.3.1_to_1.3.2_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.3.2.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.3.2-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_3_2_cache_pointer_browser_ui.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_3_2_cache_pointer_browser_ui_negative_mutations.py')],
        cwd=extracted, check=True
    )
