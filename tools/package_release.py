...
contract_path = root/'regression/1.0.11_to_1.0.12_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.12.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.12-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_12_reports_visual_navigation_ui.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_12_reports_visual_navigation_ui_negative_mutations.py')],
        cwd=extracted, check=True
    )
