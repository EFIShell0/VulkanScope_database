...
contract_path = root/'regression/1.2.0_to_1.2.1_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.1.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.1-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_1_compare_temporal_detail.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_1_compare_temporal_detail_negative_mutations.py')],
        cwd=extracted, check=True
    )
