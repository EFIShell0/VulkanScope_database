...
contract_path = root/'regression/1.2.9_to_1.2.10_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.10.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.10-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_10_mobile_filters_compare_versions.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_10_mobile_filters_compare_versions_negative_mutations.py')],
        cwd=extracted, check=True
    )
