...
contract_path = root/'regression/1.2.2_to_1.2.3_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.3.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.3-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_3_browser_motion_mobile.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_3_browser_motion_mobile_negative_mutations.py')],
        cwd=extracted, check=True
    )
