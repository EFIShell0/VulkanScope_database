...
contract_path = root/'regression/1.2.8_to_1.2.9_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.9.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.9-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_9_ui_responsive_scroll.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_9_ui_responsive_scroll_negative_mutations.py')],
        cwd=extracted, check=True
    )
