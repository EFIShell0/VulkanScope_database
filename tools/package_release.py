...
contract_path = root/'regression/1.3.9_to_1.3.10_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.3.10.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.3.10-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/audit_database.py'), '--source-tree', '.'],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_3_10_country_selector_search_pagination.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_3_10_country_selector_search_pagination_negative_mutations.py')],
        cwd=extracted, check=True
    )
