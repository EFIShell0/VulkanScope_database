...
contract_path = root/'regression/1.0.21_to_1.0.22_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.22.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.22-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_22_live_report_sync.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_22_live_report_sync_negative_mutations.py')],
        cwd=extracted, check=True
    )
