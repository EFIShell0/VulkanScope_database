...
contract_path = root/'regression/1.4.4_to_1.4.5_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.4.5.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.4.5-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_4_5_ip_action_filter_clear.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_4_5_ip_action_filter_clear_negative_mutations.py')],
        cwd=extracted, check=True
    )
