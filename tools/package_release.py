...
contract_path = root/'regression/1.2.3_to_1.2.4_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.4.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.4-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_4_ci_bounded_tables.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_4_ci_bounded_tables_negative_mutations.py')],
        cwd=extracted, check=True
    )
