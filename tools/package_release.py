...
contract_path = root/'regression/1.4.1_to_1.4.2_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.4.2.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.4.2-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_4_2_startup_layout_address_ui.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_4_2_startup_layout_address_ui_negative_mutations.py')],
        cwd=extracted, check=True
    )
