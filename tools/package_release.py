...
contract_path = root/'regression/1.0.23_to_1.0.24_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.24.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.24-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_24_regional_compare_country.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_24_regional_compare_country_negative_mutations.py')],
        cwd=extracted, check=True
    )
