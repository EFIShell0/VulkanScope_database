...
contract_path = root/'regression/1.3.2_to_1.3.3_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.3.3.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.3.3-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_3_3_compare_direction_symmetric_motion.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_3_3_compare_direction_symmetric_motion_negative_mutations.py')],
        cwd=extracted, check=True
    )
