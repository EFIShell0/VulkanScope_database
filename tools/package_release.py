...
contract_path = root/'regression/1.2.1_to_1.2.2_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.2.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.2-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_2_release_retry.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_2_release_retry_negative_mutations.py')],
        cwd=extracted, check=True
    )
