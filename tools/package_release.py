...
contract_path = root/'regression/1.2.4_to_1.2.5_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.5.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.5-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_5_scrollbar_motion.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_5_scrollbar_motion_negative_mutations.py')],
        cwd=extracted, check=True
    )
