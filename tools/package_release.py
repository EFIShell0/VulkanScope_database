...
contract_path = root/'regression/1.4.0_to_1.4.1_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.4.1.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.4.1-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_4_1_address_privacy_filter_startup.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_4_1_address_privacy_filter_startup_negative_mutations.py')],
        cwd=extracted, check=True
    )
