...
contract_path = root/'regression/1.0.22_to_1.0.23_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.23.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.23-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_23_connectivity_settings_motion.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_23_connectivity_settings_motion_negative_mutations.py')],
        cwd=extracted, check=True
    )
