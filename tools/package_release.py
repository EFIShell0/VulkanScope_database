...
contract_path = root/'regression/1.0.16_to_1.0.17_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.17.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.17-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_17_glow_versions_paged_modals.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_17_glow_versions_paged_modals_negative_mutations.py')],
        cwd=extracted, check=True
    )
