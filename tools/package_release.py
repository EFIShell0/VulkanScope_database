...
contract_path = root/'regression/1.3.7_to_1.3.8_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.3.8.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.3.8-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_3_8_regional_filter_browser_cleanup.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_3_8_regional_filter_browser_cleanup_negative_mutations.py')],
        cwd=extracted, check=True
    )
