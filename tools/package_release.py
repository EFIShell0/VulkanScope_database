...
contract_path = root/'regression/1.0.24_to_1.0.25_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.25.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.25-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_25_connectivity_surface_encyclopedia.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_25_connectivity_surface_encyclopedia_negative_mutations.py')],
        cwd=extracted, check=True
    )
