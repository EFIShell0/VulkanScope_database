...
contract_path = root/'regression/1.0.8_to_1.0.9_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.9.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.9-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_9_live_consistency_ui_floor.py')],
        cwd=extracted, check=True
    )
