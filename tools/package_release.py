...
contract_path = root/'regression/1.4.3_to_1.4.4_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.4.4.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.4.4-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_4_4_viewport_scroll_progress.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_4_4_viewport_scroll_progress_negative_mutations.py')],
        cwd=extracted, check=True
    )
