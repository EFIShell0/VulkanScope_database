...
contract_path = root/'regression/1.0.18_to_1.0.19_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.19.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.19-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_19_dominant_glow_release_ready.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_19_dominant_glow_release_ready_negative_mutations.py')],
        cwd=extracted, check=True
    )
