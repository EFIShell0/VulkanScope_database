...
contract_path = root/'regression/1.2.7_to_1.2.8_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.2.8.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.2.8-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_2_8_compare_fixed_follow.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_2_8_compare_fixed_follow_negative_mutations.py')],
        cwd=extracted, check=True
    )
