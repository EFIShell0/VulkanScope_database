...
contract_path = root/'regression/1.4.2_to_1.4.3_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.4.3.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.4.3-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_4_3_startup_loader_address_fit.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_4_3_startup_loader_address_fit_negative_mutations.py')],
        cwd=extracted, check=True
    )
