...
contract_path = root/'regression/1.0.10_to_1.0.11_contract.json'
...
zip_path = out_dir / 'VulkanScope-Database-1.0.11.zip'
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-release-1.0.11-') as td:
    ...
    subprocess.run(
        [sys.executable, str(extracted/'tools/verify_1_0_11_workflow_command_boundary.py')],
        cwd=extracted, check=True
    )
    subprocess.run(
        [sys.executable, str(extracted/'tools/test_1_0_11_workflow_command_boundary_negative_mutations.py')],
        cwd=extracted, check=True
    )
