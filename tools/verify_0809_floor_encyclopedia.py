...
app_path=root/'assets/app.v1005.js'; enc_path=root/'assets/encyclopedia.v03924.js'
...
if not a.skip_version:
    if 'VulkanScope Database <strong>1.0.5</strong>' not in index: errors.append('Database 1.0.5 footer missing')
    if pkg.get('version')!='1.0.5': errors.append('Worker package version must be 1.0.1')
if './assets/encyclopedia.v03924.js' not in index or './assets/app.v1005.js' not in index: errors.append('0.39.25 Encyclopedia/app assets are not loaded')
...
print('PASS Database 1.0.5 retained floor/Encyclopedia contract')
