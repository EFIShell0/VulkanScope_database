...
app_path=root/'assets/app.v03925.js'; enc_path=root/'assets/encyclopedia.v03924.js'
...
if not a.skip_version:
    if 'VulkanScope Database <strong>0.39.25</strong>' not in index: errors.append('Database 0.39.25 footer missing')
    if pkg.get('version')!='0.39.25': errors.append('Worker package version must be 0.39.25')
if './assets/encyclopedia.v03924.js' not in index or './assets/app.v03925.js' not in index: errors.append('0.39.25 Encyclopedia/app assets are not loaded')
...
if errors: raise SystemExit('FAIL Database 0.39.25 floor/Encyclopedia contract\n- '+'\n- '.join(errors))
print('PASS Database 0.39.25 floor/Encyclopedia contract')
