...
worker=txt('worker/src/index.js'); tests=txt('worker/tests/contract.mjs'); index=txt('index.html'); app=txt('assets/app.v1006.js'); rules=txt('rules/PROJECT_RULES.md'); audit=txt('rules/1.0.1_FULL_SECURITY_SPEC_CORRECTNESS_USABILITY_AUDIT.md'); static=js('data/index.json'); pkg=js('worker/package.json')
if not a.skip_version:
    ...
    need((('app.v1006.js?v=1001' in index and 'config.js?v=1001' in index) or ('app.v1006.js?v=1006' in index and 'config.js?v=1002' in index)),'retained cache identity missing')
