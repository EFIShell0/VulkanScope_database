...
if not args.skip_version: need(package.get('version')=='1.0.5','Database version is not 1.0.5')
...
index=text('index.html'); need('Database <strong>1.0.5</strong>' in index and 'encyclopedia.v03924.js' in index and 'app.v1005.js?v=1005' in index,'index asset/version identity not 1.0.5')
worker=text('worker/src/index.js'); need("VulkanScope 1.0.15 · Vulkan 1.4.362" in worker,'Worker producer/query baseline not 1.0.5 / 1.4.362'); need('Vulkan 1.4.362 (2026-09-04)' in worker,'Worker published Vulkan spec not 1.4.362'); need('VulkanScope 0.80.3 or newer is required for new submissions' in worker,'0.80.3 producer floor drifted'); need('producerAtLeast08010' in worker and "registryVersion=producerAtLeast08010(p)?'1.4.362':'1.4.361'" in worker and 'baseline=`Vulkan ${registryVersion}`' in worker,'Worker does not preserve exact producer registry-baseline strings for 0.80.3-0.80.9 and 0.80.10+')
app=text('assets/app.v1005.js'); need('VulkanScope 1.0.15 · Vulkan 1.4.362' in app and 'Vulkan 1.4.362 (2026-09-04)' in app,'frontend baseline not 1.0.5 / 1.4.362'); need('Vulkan 1.4.362 reference' in app and 'locked Vulkan 1.4.362 registry' in app and 'Vulkan 1.4.361 reference' not in app,'Database Encyclopedia visible copy is stale')
...
if errors:
    print('FAIL Database 0.39.27 retained Vulkan 1.4.362 contract')
    for e in errors:print('-',e)
    raise SystemExit(1)
print('PASS Database 1.0.5 / Vulkan 1.4.362 retained contract: databaseRelease=1.0.5 floor=0.80.3 symbols=842/6248/2461 extensions=476')
