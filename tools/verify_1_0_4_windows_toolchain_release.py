...
parser = argparse.ArgumentParser(description='Verify VulkanScope Database 1.0.8 Windows/toolchain/release hardening')
...
app = text('assets/app.v1008.js')
need('VulkanScope Database <strong>1.0.8</strong>' in index, '1.0.8 footer identity missing')
need('app.v1008.js?v=1008' in index and 'config.js?v=1008' in index and 'site.v0390.css?v=1008' in index, '1.0.8 cache identity missing')
need('Database 1.0.8 · schema' in app, 'frontend Database 1.0.8 identity missing')
...
if errors:
    print('FAIL VulkanScope Database 1.0.8 Windows/toolchain/release audit')
    for e in errors:
        print(' - ' + e)
    raise SystemExit(1)
print('PASS VulkanScope Database 1.0.8: Windows-safe Node paths, reviewed install-script policy, Wrangler 4.130.0, automatic tag release')
