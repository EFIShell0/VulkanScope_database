...
parser = argparse.ArgumentParser(description='Verify VulkanScope Database 1.0.7 Windows/toolchain/release hardening')
...
if security_cmd=='node scripts/security-audit.mjs':
    ...
    need("runNpm(['audit','--audit-level=high'])" in helper or "run(['audit','--audit-level=high'])" in helper, 'lock-backed security helper no longer enforces high-severity npm audit')
...
app = text('assets/app.v1007.js')
need('VulkanScope Database <strong>1.0.7</strong>' in index, '1.0.7 footer identity missing')
need('app.v1007.js?v=1007' in index and 'config.js?v=1007' in index and 'site.v0390.css?v=1007' in index, '1.0.7 cache identity missing')
need('Database 1.0.7 · schema' in app, 'frontend Database 1.0.7 identity missing')
...
if errors:
    print('FAIL VulkanScope Database 1.0.7 Windows/toolchain/release audit')
    for e in errors:
        print(' - ' + e)
    raise SystemExit(1)
print('PASS VulkanScope Database 1.0.7: Windows-safe Node paths, reviewed install-script policy, Wrangler 4.130.0, automatic tag release')
