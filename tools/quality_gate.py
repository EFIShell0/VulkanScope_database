...
run(sys.executable,'tools/verify_1_3_2_cache_pointer_browser_ui.py')
run(sys.executable,'tools/test_1_3_2_cache_pointer_browser_ui_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1302.js')
run('node','--check','assets/browser-compat.v1302.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/app.v1301.js').is_file()
    assert (staged/'assets/release-bootstrap.v1302.js').is_file()
    assert (staged/'assets/release-bootstrap.v1301.js').is_file()
...
print('VulkanScope Database 1.3.2 quality gate: PASS')
