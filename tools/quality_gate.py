...
run(sys.executable,'tools/verify_1_0_11_workflow_command_boundary.py')
run(sys.executable,'tools/test_1_0_11_workflow_command_boundary_negative_mutations.py')
run('node','--check','assets/app.v1011.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1011.js').is_file()
...
print('VulkanScope Database 1.0.11 quality gate: PASS')
