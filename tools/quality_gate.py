...
run(sys.executable,'tools/verify_1_2_0_workspace_floor_settings.py')
run(sys.executable,'tools/test_1_2_0_workspace_floor_settings_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1200.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1200.js').is_file()
...
print('VulkanScope Database 1.2.0 quality gate: PASS')
