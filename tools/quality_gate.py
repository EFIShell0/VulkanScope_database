...
run(sys.executable,'tools/verify_1_0_15_database_ui_integrity.py')
run(sys.executable,'tools/test_1_0_15_database_ui_integrity_negative_mutations.py')
run('node','--check','assets/app.v1015.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1015.js').is_file()
...
print('VulkanScope Database 1.0.15 quality gate: PASS')
