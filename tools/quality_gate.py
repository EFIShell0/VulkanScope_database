...
run(sys.executable,'tools/verify_1_2_5_scrollbar_motion.py')
run(sys.executable,'tools/test_1_2_5_scrollbar_motion_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1205.js')
run('node','--check','assets/browser-compat.v1205.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1205.js').is_file()
    assert (staged/'assets/browser-compat.v1205.js').is_file()
...
print('VulkanScope Database 1.2.5 quality gate: PASS')
