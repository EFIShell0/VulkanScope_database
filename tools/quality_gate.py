...
run(sys.executable,'tools/verify_1_3_9_regional_filter_layout_profile.py')
run(sys.executable,'tools/test_1_3_9_regional_filter_layout_profile_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1309.js')
run('node','--check','assets/browser-compat.v1309.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/app.v1308.js').is_file()
    assert not (staged/'assets/app.v1307.js').exists()
    assert (staged/'assets/release-bootstrap.v1309.js').is_file()
    assert (staged/'assets/release-bootstrap.v1308.js').is_file()
    assert not (staged/'assets/release-bootstrap.v1307.js').exists()
    assert (staged/'assets/browser-compat.v1309.js').is_file()
    assert (staged/'assets/browser-compat.v1308.js').is_file()
    assert not (staged/'assets/browser-compat.v1307.js').exists()
...
print('VulkanScope Database 1.3.9 quality gate: PASS')
