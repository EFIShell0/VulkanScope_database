...
run(sys.executable,'tools/verify_1_3_3_compare_direction_symmetric_motion.py')
run(sys.executable,'tools/test_1_3_3_compare_direction_symmetric_motion_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1303.js')
run('node','--check','assets/browser-compat.v1303.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1303.js').is_file()
    assert (staged/'assets/app.v1302.js').is_file()
    assert (staged/'assets/release-bootstrap.v1303.js').is_file()
    assert (staged/'assets/release-bootstrap.v1302.js').is_file()
    assert (staged/'assets/browser-compat.v1303.js').is_file()
    assert (staged/'assets/browser-compat.v1302.js').is_file()
...
print('VulkanScope Database 1.3.3 quality gate: PASS')
