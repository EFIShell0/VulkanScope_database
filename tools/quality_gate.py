...
run(sys.executable,'tools/verify_1_0_12_reports_visual_navigation_ui.py')
run(sys.executable,'tools/test_1_0_12_reports_visual_navigation_ui_negative_mutations.py')
run('node','--check','assets/app.v1012.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1012.js').is_file()
...
print('VulkanScope Database 1.0.12 quality gate: PASS')
