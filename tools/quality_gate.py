...
run(sys.executable,'tools/verify_1_0_23_connectivity_settings_motion.py')
run(sys.executable,'tools/test_1_0_23_connectivity_settings_motion_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1023.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1023.js').is_file()
...
print('VulkanScope Database 1.0.23 quality gate: PASS')
