...
run(sys.executable,'tools/verify_1_0_22_live_report_sync.py')
run(sys.executable,'tools/test_1_0_22_live_report_sync_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1022.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1022.js').is_file()
...
print('VulkanScope Database 1.0.22 quality gate: PASS')
