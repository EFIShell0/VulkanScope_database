...
run(sys.executable,'tools/verify_1_3_8_regional_filter_browser_cleanup.py')
run(sys.executable,'tools/test_1_3_8_regional_filter_browser_cleanup_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1308.js')
run('node','--check','assets/browser-compat.v1308.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/app.v1307.js').is_file()
    assert (staged/'assets/release-bootstrap.v1308.js').is_file()
    assert (staged/'assets/release-bootstrap.v1307.js').is_file()
    assert (staged/'assets/browser-compat.v1308.js').is_file()
    assert (staged/'assets/browser-compat.v1307.js').is_file()
...
print('VulkanScope Database 1.3.8 quality gate: PASS')
