...
run(sys.executable,'tools/verify_1_0_19_dominant_glow_release_ready.py')
run(sys.executable,'tools/test_1_0_19_dominant_glow_release_ready_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1019.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1019.js').is_file()
...
print('VulkanScope Database 1.0.19 quality gate: PASS')
