...
run(sys.executable,'tools/verify_1_3_7_browser_brand_marks.py')
run(sys.executable,'tools/test_1_3_7_browser_brand_marks_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1307.js')
run('node','--check','assets/browser-compat.v1307.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/app.v1306.js').is_file()
    assert (staged/'assets/release-bootstrap.v1307.js').is_file()
    assert (staged/'assets/release-bootstrap.v1306.js').is_file()
    assert (staged/'assets/browser-compat.v1307.js').is_file()
    assert (staged/'assets/browser-compat.v1306.js').is_file()
...
print('VulkanScope Database 1.3.7 quality gate: PASS')
