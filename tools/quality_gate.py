...
run(sys.executable,'tools/verify_1_0_13_reports_disclosure_spray_ui.py')
run(sys.executable,'tools/test_1_0_13_reports_disclosure_spray_ui_negative_mutations.py')
run('node','--check','assets/app.v1013.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1013.js').is_file()
...
print('VulkanScope Database 1.0.13 quality gate: PASS')
