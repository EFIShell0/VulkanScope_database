...
run(sys.executable,'tools/verify_1_4_5_ip_action_filter_clear.py')
run(sys.executable,'tools/test_1_4_5_ip_action_filter_clear_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1405.js')
run('node','--check','assets/browser-compat.v1405.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/app.v1404.js').is_file()
    assert not (staged/'assets/app.v1403.js').exists()
    assert (staged/'assets/release-bootstrap.v1405.js').is_file()
    assert (staged/'assets/release-bootstrap.v1404.js').is_file()
    assert not (staged/'assets/release-bootstrap.v1403.js').exists()
    assert (staged/'assets/browser-compat.v1405.js').is_file()
    assert (staged/'assets/browser-compat.v1404.js').is_file()
    assert not (staged/'assets/browser-compat.v1403.js').exists()
...
print('VulkanScope Database 1.4.5 quality gate: PASS')
