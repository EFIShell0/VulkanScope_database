...
run(sys.executable,'tools/verify_1_2_2_release_retry.py')
run(sys.executable,'tools/test_1_2_2_release_retry_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1202.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1202.js').is_file()
...
print('VulkanScope Database 1.2.2 quality gate: PASS')
