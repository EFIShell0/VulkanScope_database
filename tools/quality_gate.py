...
run(sys.executable,'tools/verify_1_3_1_freshness_filters_license_live_sync.py')
run(sys.executable,'tools/test_1_3_1_freshness_filters_license_live_sync_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1301.js')
run('node','--check','assets/browser-compat.v1301.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/browser-compat.v1301.js').is_file()
...
print('VulkanScope Database 1.3.1 quality gate: PASS')
