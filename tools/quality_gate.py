...
run(sys.executable,'tools/verify_1_4_0_submission_settings_ip_filter_boot.py')
run(sys.executable,'tools/test_1_4_0_submission_settings_ip_filter_boot_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1400.js')
run('node','--check','assets/browser-compat.v1400.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/app.v1310.js').is_file()
    assert not (staged/'assets/app.v1309.js').exists()
    assert (staged/'assets/release-bootstrap.v1400.js').is_file()
    assert (staged/'assets/release-bootstrap.v1310.js').is_file()
    assert not (staged/'assets/release-bootstrap.v1309.js').exists()
    assert (staged/'assets/browser-compat.v1400.js').is_file()
    assert (staged/'assets/browser-compat.v1310.js').is_file()
    assert not (staged/'assets/browser-compat.v1309.js').exists()
...
print('VulkanScope Database 1.4.0 quality gate: PASS')
