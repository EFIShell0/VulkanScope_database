...
run(sys.executable,'tools/verify_1_2_3_browser_motion_mobile.py')
run(sys.executable,'tools/test_1_2_3_browser_motion_mobile_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1203.js')
run('node','--check','assets/browser-compat.v1203.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1203.js').is_file()
    assert (staged/'assets/browser-compat.v1203.js').is_file()
...
print('VulkanScope Database 1.2.3 quality gate: PASS')
