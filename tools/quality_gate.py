...
run(sys.executable,'tools/verify_1_0_16_live_modal_versions.py')
run(sys.executable,'tools/test_1_0_16_live_modal_versions_negative_mutations.py')
run('node','--check','assets/app.v1016.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1016.js').is_file()
...
print('VulkanScope Database 1.0.16 quality gate: PASS')
