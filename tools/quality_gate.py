...
run(sys.executable,'tools/verify_1_3_10_country_selector_search_pagination.py')
run(sys.executable,'tools/test_1_3_10_country_selector_search_pagination_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1310.js')
run('node','--check','assets/browser-compat.v1310.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/app.v1309.js').is_file()
    assert not (staged/'assets/app.v1308.js').exists()
    assert (staged/'assets/release-bootstrap.v1310.js').is_file()
    assert (staged/'assets/release-bootstrap.v1309.js').is_file()
    assert not (staged/'assets/release-bootstrap.v1308.js').exists()
    assert (staged/'assets/browser-compat.v1310.js').is_file()
    assert (staged/'assets/browser-compat.v1309.js').is_file()
    assert not (staged/'assets/browser-compat.v1308.js').exists()
...
print('VulkanScope Database 1.3.10 quality gate: PASS')
