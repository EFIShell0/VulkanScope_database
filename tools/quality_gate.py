...
run(sys.executable,'tools/verify_1_2_9_ui_responsive_scroll.py')
run(sys.executable,'tools/test_1_2_9_ui_responsive_scroll_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1209.js')
run('node','--check','assets/browser-compat.v1209.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1209.js').is_file()
    assert (staged/'assets/browser-compat.v1209.js').is_file()
...
print('VulkanScope Database 1.2.9 quality gate: PASS')
