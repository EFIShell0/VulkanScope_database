...
run(sys.executable,'tools/verify_1_4_4_viewport_scroll_progress.py')
run(sys.executable,'tools/test_1_4_4_viewport_scroll_progress_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1404.js')
run('node','--check','assets/browser-compat.v1404.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/app.v1403.js').is_file()
    assert not (staged/'assets/app.v1402.js').exists()
    assert (staged/'assets/release-bootstrap.v1404.js').is_file()
    assert (staged/'assets/release-bootstrap.v1403.js').is_file()
    assert not (staged/'assets/release-bootstrap.v1402.js').exists()
    assert (staged/'assets/browser-compat.v1404.js').is_file()
    assert (staged/'assets/browser-compat.v1403.js').is_file()
    assert not (staged/'assets/browser-compat.v1402.js').exists()
...
print('VulkanScope Database 1.4.4 quality gate: PASS')
