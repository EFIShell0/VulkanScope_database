...
run(sys.executable,'tools/verify_1_3_6_report_detail_workspace_raw_download.py')
run(sys.executable,'tools/test_1_3_6_report_detail_workspace_raw_download_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1306.js')
run('node','--check','assets/browser-compat.v1306.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    assert (staged/'assets/app.v1305.js').is_file()
    assert (staged/'assets/release-bootstrap.v1306.js').is_file()
    assert (staged/'assets/release-bootstrap.v1305.js').is_file()
    assert (staged/'assets/browser-compat.v1306.js').is_file()
    assert (staged/'assets/browser-compat.v1305.js').is_file()
    assert (staged/'assets/site.v0390.css').is_file()
    assert (staged/'assets/hdr/hdr10_plus_v1014.png').is_file()
    for name in ('wrangler.md','sharp.md','esbuild.md','workerd.md','nodejs.md','python.md'):
...
print('VulkanScope Database 1.3.6 quality gate: PASS')
