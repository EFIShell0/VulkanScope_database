...
run(sys.executable,'tools/verify_1_0_24_regional_compare_country.py')
run(sys.executable,'tools/test_1_0_24_regional_compare_country_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1024.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1024.js').is_file()
...
print('VulkanScope Database 1.0.24 quality gate: PASS')
