...
run(sys.executable,'tools/verify_1_0_25_connectivity_surface_encyclopedia.py')
run(sys.executable,'tools/test_1_0_25_connectivity_surface_encyclopedia_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1025.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1025.js').is_file()
...
print('VulkanScope Database 1.0.25 quality gate: PASS')
