...
run(sys.executable,'tools/verify_1_0_17_glow_versions_paged_modals.py')
run(sys.executable,'tools/test_1_0_17_glow_versions_paged_modals_negative_mutations.py')
run('node','--check','assets/app.v1017.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1017.js').is_file()
...
print('VulkanScope Database 1.0.17 quality gate: PASS')
