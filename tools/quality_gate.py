...
run(sys.executable,'tools/verify_1_0_10_reports_render_hotfix.py')
run(sys.executable,'tools/test_1_0_10_reports_render_hotfix_negative_mutations.py')
run('node','--check','assets/app.v1010.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1010.js').is_file()
...
print('VulkanScope Database 1.0.10 quality gate: PASS')
