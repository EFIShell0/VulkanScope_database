...
run(sys.executable,'tools/verify_1_3_0_trademark_licenses_full_audit.py')
run(sys.executable,'tools/test_1_3_0_trademark_licenses_full_audit_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1300.js')
run('node','--check','assets/browser-compat.v1300.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1300.js').is_file()
    assert (staged/'assets/browser-compat.v1300.js').is_file()
...
print('VulkanScope Database 1.3.0 quality gate: PASS')
