...
# Immutable 1.0.8 hashes protect every untouched predecessor path; this release-specific
# gate exercises the deliberately changed live/UI/producer-floor surface plus core invariants.
...
run(sys.executable,'tools/verify_1_0_9_live_consistency_ui_floor.py')
run('node','--check','assets/app.v1009.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1009.js').is_file()
...
print('VulkanScope Database 1.0.9 quality gate: PASS')
