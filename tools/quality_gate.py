...
run(sys.executable,'tools/verify_1_2_10_mobile_filters_compare_versions.py')
run(sys.executable,'tools/test_1_2_10_mobile_filters_compare_versions_negative_mutations.py')
run(sys.executable,'tools/audit_database.py','--source-tree','.')
run('node','--check','assets/app.v1210.js')
run('node','--check','assets/browser-compat.v1210.js')
...
with tempfile.TemporaryDirectory(prefix='vulkanscope-db-pages-') as td:
    ...
    staged=Path(td)
    assert (staged/'assets/app.v1210.js').is_file()
    assert (staged/'assets/browser-compat.v1210.js').is_file()
...
print('VulkanScope Database 1.2.10 quality gate: PASS')
