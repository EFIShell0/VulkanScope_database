...
const appPath='assets/app.v1003.js';
need(fs.existsSync(path.join(root,appPath)),'current frontend asset missing');
need(index.includes('VulkanScope Database <strong>1.0.3</strong>'),'index database version stale');
need(index.includes('./config.js?v=1003'),'config cache key stale');
need(index.includes('./assets/app.v1003.js'),'frontend asset cache key stale');
if(fs.existsSync(path.join(root,appPath))){
  ...
  need(app.includes('Database 1.0.3 · schema'),'frontend footer database version stale');
}
...
need(packageJson.version==='1.0.3','Worker package version stale');
const buildIndex=read('tools/build_index.py');
need(buildIndex.includes('"databaseVersion":"1.0.3"'),'build_index database version stale');
...
console.log('PASS retained 0.41.46 producer-baseline surface on Database 1.0.3');
