...
const appPath='assets/app.v03925.js';
need(fs.existsSync(path.join(root,appPath)),'current frontend asset missing');
need(index.includes('VulkanScope Database <strong>0.39.25</strong>'),'index database version stale');
need(index.includes('./config.js?v=03925'),'config cache key stale');
need(index.includes('./assets/app.v03925.js'),'frontend asset cache key stale');
if(fs.existsSync(path.join(root,appPath))){
  ...
  need(app.includes('Database 0.39.25 · schema'),'frontend footer database version stale');
}
...
need(packageJson.version==='0.39.25','Worker package version stale');
const buildIndex=read('tools/build_index.py');
need(buildIndex.includes('"databaseVersion":"0.39.25"'),'build_index database version stale');
...
console.log('PASS VulkanScope Database 0.39.25 producer baseline for VulkanScope 0.80.9');
