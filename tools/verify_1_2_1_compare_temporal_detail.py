...
app=text('assets/app.v1201.js'); css=text('assets/site.v0390.css'); index=text('index.html'); worker=text('worker/src/index.js'); rules=text('rules/PROJECT_RULES.md'); contract=text('worker/tests/contract.mjs')
need("const DATABASE_VERSION='1.2.1',LIVE_SYNC_INTERVAL_MS=3000,RELEASE_CHECK_INTERVAL_MS=10000,NETWORK_INFO_INTERVAL_MS=3000,NETWORK_PROBE_TIMEOUT_MS=2200;" in app,'1.2.1 frontend identity/timers missing')
need('assets/app.v1201.js?v=1201' in index and 'site.v0390.css?v=1201' in index and 'config.js?v=1201' in index,'1.2.1 cache-busted references missing')
need('VulkanScope Database <strong>1.2.1</strong>' in index,'1.2.1 footer identity missing')
...
need('## Release 1.2.1 comparison tracking / temporal integrity / confirmation / report-detail requirements' in rules,'1.2.1 rules section missing')
marker=json.loads(text('data/release.json') or '{}')
need(marker=={'schemaVersion':2,'databaseVersion':'1.2.1','releaseReady':False,'appAsset':'assets/app.v1201.js','cacheKey':'1201'},'source release marker mismatch')
if errors:
    print('\n'.join('FAIL '+e for e in errors)); raise SystemExit(1)
print('PASS VulkanScope Database 1.2.1 compare/temporal/detail contract')
