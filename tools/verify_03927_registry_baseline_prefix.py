...
app=text('assets/app.v1003.js')
package=json.loads(text('worker/package.json'))
need(package.get('version')=='1.0.3','Database/Worker version is not 1.0.3')
need('VulkanScope Database <strong>1.0.3</strong>' in index and 'app.v1003.js?v=1003' in index,'1.0.3 browser identity/cache-bust missing')
need('Database 1.0.3 · schema' in app,'1.0.3 frontend release identity missing')
...
if errors:
    print('FAIL Database 1.0.3 retained registry-baseline producer-string contract')
    for e in errors: print('-',e)
    raise SystemExit(1)
print('PASS Database 1.0.3 retained exact registry-baseline strings: 0.80.3-0.80.9=Vulkan 1.4.361; 0.80.10+=Vulkan 1.4.362')
