...
worker=txt('worker/src/index.js'); tests=txt('worker/tests/contract.mjs'); index=txt('index.html'); app=txt('assets/app.v1003.js'); rules=txt('rules/PROJECT_RULES.md'); audit=txt('rules/1.0.2_VULKANSCOPE_1.0.2_PRODUCER_BASELINE_AUDIT.md'); static=js('data/index.json'); pkg=js('worker/package.json')
if not a.skip_version:
    ...
    need('app.v1003.js?v=1003' in index and 'config.js?v=1002' in index and 'site.v0390.css?v=1002' in index,'1.0.2 cache identity missing')
...
need(apps==['app.v1003.js'],f'strict current frontend asset identity drifted: {apps}')
if errors:
