...
ASSET_ALLOW={
    APP_ASSET,'browser-compat.v1207.js','encyclopedia.v03924.js','site.v0390.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png','favicon.ico','favicon.png','vulkanscope_logo_horizontal.png',
...
def audit_source(root:Path):
    ...
    if not root.is_dir():
        print('\n'.join('FAIL '+e for e in errors)); raise SystemExit(1)
    required=['index.html','config.js','report.schema.json',f'assets/{APP_ASSET}','assets/browser-compat.v1207.js','assets/site.v0390.css','assets/encyclopedia.v03924.js','worker/src/index.js','worker/package.json','worker/wrangler.jsonc','worker/tests/contract.mjs','rules/PROJECT_RULES.md','tools/quality_gate.py','tools/repair_repository.py','tools/mark_release_ready.py','tools/verify_1_2_1_compare_temporal_detail.py','tools/verify_1_2_7_compare_hero_scroll.py','tools/test_1_2_7_compare_hero_scroll_negative_mutations.py','tools/pages.workflow.yml','.github/workflows/pages.yml','registry/registry_lock.json','registry/upstream/vk.xml']
    for rel in required: need((root/rel).is_file(),f'missing required source file {rel}')
    if errors:
        print('\n'.join('FAIL '+e for e in errors)); raise SystemExit(1)
    index=text('index.html'); app=text(f'assets/{APP_ASSET}'); compat=text('assets/browser-compat.v1207.js'); css=text('assets/site.v0390.css'); worker=text('worker/src/index.js'); rules=text('rules/PROJECT_RULES.md'); workflow=text('.github/workflows/pages.yml'); workflow_template=text('tools/pages.workflow.yml')
    ...
    need('browser-compat.v1207.js?v=1207' in index and 'browserCompatibilityGate' in index,'browser compatibility gate/reference missing')
    ...
    need(worker.count("databaseReleaseVersion:'1.2.7'")>=3,'Worker databaseReleaseVersion mismatch')
    need(worker.count("workerReleaseVersion:'1.2.7'")>=3,'Worker workerReleaseVersion mismatch')
    ...
    if node:
        for rel in [f'assets/{APP_ASSET}','assets/browser-compat.v1207.js','assets/encyclopedia.v03924.js','worker/src/index.js','worker/tests/contract.mjs']:
