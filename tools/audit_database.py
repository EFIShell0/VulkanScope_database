...
app=(root/'assets/app.v0359.js').read_text()
worker=(root/'worker/src/index.js').read_text()
rules=(root/'rules/PROJECT_RULES.md').read_text()
check('VulkanScope Database <strong>0.35.9</strong>' in index,'index version')
check('site.v0357.css' in index and 'app.v0359.js' in index and 'config.js?v=0359' in index,'cache-busted asset refs')
check("connect-src 'self' https://vulkanscope-database-api.vulkanscope.workers.dev" in index,'CSP API pin')
check('Database 0.35.9' in app,'frontend version')
...
check('Release 0.35.9 Display & HDR / ABI / home-metric parity' in rules,'release rules')
...
if errors:
    ...
    sys.exit(1)
print('VulkanScope Database 0.35.9 audit: PASS')
