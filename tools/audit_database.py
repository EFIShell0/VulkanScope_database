...
def audit_artifact_tree(artifact_root: Path):
    ...
    allowed_assets={
        'app.v1003.js','encyclopedia.v03924.js','site.v0390.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png',
    ...
    if idx.is_file():
        ...
        acheck('app.v1003.js' in body and 'encyclopedia.v03924.js' in body and 'config.js?v=1003' in body and 'site.v0390.css' in body,'Pages artifact current asset references')
    ...
    if artifact_errors:
        print("\n".join(artifact_errors)); sys.exit(1)
    print('VulkanScope Database 1.0.3 Pages artifact audit: PASS')
    sys.exit(0)
...
app=text(root/'assets/app.v1003.js')
...
check('VulkanScope Database <strong>1.0.3</strong>' in index,'index version')
check('site.v0390.css' in index and 'app.v1003.js' in index and 'encyclopedia.v03924.js' in index and 'config.js?v=1003' in index,'1.0.3 cache-busted asset refs')
check('site.v0390.css?v=1003' in index and 'app.v1003.js?v=1003' in index,'1.0.3 UI cache key')
check('Database 1.0.3' in app,'frontend database version')
...
check('node --check assets/app.v1003.js' in workflow,'workflow frontend syntax check')
...
check(static.get('databaseVersion')=='1.0.3','static database version')
...
check('\"databaseVersion\":\"1.0.3\"' in build_index and '\"producerQueryBaseline\":\"VulkanScope 1.0.15 · Vulkan 1.4.362\"' in build_index,'build_index current release metadata')
...
check(pkg.get('version')=='1.0.3','worker package version')
...
check(versioned_apps==['app.v1003.js'],f'exactly one versioned frontend app asset is permitted; run repository repair: {versioned_apps}')
...
if node:
    for f in [root/'assets/app.v1003.js',root/'assets/encyclopedia.v03924.js',root/'worker/src/index.js',root/'worker/tests/contract.mjs']:
