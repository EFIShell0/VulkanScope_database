...
app=(root/'assets/app.v0358.js').read_text()
worker=(root/'worker/src/index.js').read_text()
rules=(root/'rules/PROJECT_RULES.md').read_text()
check('VulkanScope Database <strong>0.35.8</strong>' in index,'index version')
check('site.v0357.css' in index and 'app.v0358.js' in index and 'config.js?v=0358' in index,'cache-busted asset refs')
check("connect-src 'self' https://vulkanscope-database-api.vulkanscope.workers.dev" in index,'CSP API pin')
check('Database 0.35.8' in app,'frontend version')
...
check('Release 0.35.8 exact Properties / Limits semantics' in rules,'release rules')
...
if errors:
    ...
    sys.exit(1)
print('VulkanScope Database 0.35.8 audit: PASS')
