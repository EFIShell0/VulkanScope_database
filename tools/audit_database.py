...
def audit_artifact_tree(artifact_root: Path):
    ...
    allowed_assets={
        'app.v03917.js','site.v0390.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png',
    ...
    if idx.is_file():
        ...
        acheck('app.v03917.js' in body and 'config.js?v=03917' in body and 'site.v0390.css' in body,'Pages artifact current asset references')
    ...
    if artifact_errors:
        print("\n".join(artifact_errors)); sys.exit(1)
    print('VulkanScope Database 0.39.17 Pages artifact audit: PASS')
    sys.exit(0)
...
app=text(root/'assets/app.v03917.js')
...
check('VulkanScope Database <strong>0.39.17</strong>' in index,'index version')
check('site.v0390.css' in index and 'app.v03917.js' in index and 'config.js?v=03917' in index,'0.39.17 cache-busted asset refs')
check('Database 0.39.17' in app,'frontend database version')
check('VulkanScope 0.41.42 · Vulkan 1.4.361' in app,'frontend producer baseline')
check("connect-src 'self' https://vulkanscope-database-api.vulkanscope.workers.dev" in index,'CSP API pin')
check('node --check assets/app.v03917.js' in workflow,'workflow frontend syntax check')
...
check('historicalPropertyFeatureCompareIdentity' in app and 'producerBefore04140' in app,'0.39.17 historical property/feature Compare identity')
check("const visibleMetricLabel=diff?'Visible differences':'Visible fields'" in app,'0.39.17 Compare visible metric semantics')
check("!['true','false'].includes(v.toLowerCase())" in app,'0.39.17 historical mapping is boolean-only')
check('surfacePresentationState' in app and 'surfaceQueueState' in app,'0.39.17 Surface evidence-state helpers')
check("put('SURFACE',k,canonicalSurfaceValue(k,v),availabilityStatus(v))" in app,'0.39.17 generic Surface values use availability semantics')
check("queryStatus:d.surface.queryStatus??'unknown'" in app,'0.39.17 structured Surface queryStatus preservation')
...
check('VulkanScope 0.41.42 · Vulkan 1.4.361' in worker,'current producer metadata')
...
check(static.get('databaseVersion')=='0.39.17','static database version')
check(static.get('normalizerVersion')==16,'static normalizer')
check(static.get('producerQueryBaseline')=='VulkanScope 0.41.42 · Vulkan 1.4.361','static producer baseline')
build_index=text(root/'tools/build_index.py')
check('\"databaseVersion\":\"0.39.17\"' in build_index and '\"producerQueryBaseline\":\"VulkanScope 0.41.42 · Vulkan 1.4.361\"' in build_index,'build_index current release metadata')
...
check(pkg.get('version')=='0.39.17','worker package version')
...
check(versioned_apps==['app.v03917.js'],f'exactly one versioned frontend app asset is permitted; run repository repair: {versioned_apps}')
...
if node:
    for f in [root/'assets/app.v03917.js',root/'worker/src/index.js',root/'worker/tests/contract.mjs']:
