...
ASSET_ALLOW={
    'app.v1402.js','browser-compat.v1402.js','release-bootstrap.v1402.js',APP_ASSET,'browser-compat.v1403.js','release-bootstrap.v1403.js','encyclopedia.v03924.js','site.v0390.css','site.v1308.css','site.v1309.css','site.v1400.css','site.v1401.css','site.v1402.css','site.v1403.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png','favicon.ico','favicon.png','vulkanscope_logo_horizontal.png','browser-logos/google-chrome.v1307.svg','browser-logos/chromium.v1307.png','browser-logos/microsoft-edge.v1307.svg','browser-logos/firefox.v1307.svg','browser-logos/safari.v1307.svg','browser-logos/opera.v1307.svg','browser-logos/brave.v1307.svg','browser-logos/vivaldi.v1307.svg','browser-logos/samsung-browser.v1307.svg',
...
def audit_artifact(root:Path,require_release_ready=False):
    ...
    for token in [f'assets/{APP_ASSET}?v={CACHE_KEY}','site.v1403.css?v=1403',f'config.js?v={CACHE_KEY}',f'VulkanScope Database <strong>{DB_VERSION}</strong>']:
...
def audit_source(root:Path):
    ...
    for token in [f'VulkanScope Database <strong>{DB_VERSION}</strong>',f'assets/{APP_ASSET}?v={CACHE_KEY}','site.v1403.css?v=1403',f'config.js?v={CACHE_KEY}']:
    ...
    need('browser-compat.v1403.js?v=1403' in index and 'release-bootstrap.v1403.js?v=1403' in index and 'browserCompatibilityGate' in index,'browser compatibility/startup freshness references missing')
    ...
    boot=text('assets/release-bootstrap.v1403.js')
    ...
    need(worker.count("databaseReleaseVersion:'1.4.3'")>=3,'Worker databaseReleaseVersion mismatch')
    need(worker.count("workerReleaseVersion:'1.4.3'")>=3,'Worker workerReleaseVersion mismatch')
    ...
    need('## Release 1.4.3 startup-loader isolation / address-fit requirements' in rules,'1.4.3 rules section missing')
    need('<section id="databaseLoading" class="database-loading-panel" role="status" aria-live="polite" aria-atomic="true">' in index,'1.4.3 loading panel must use a presentation class distinct from the body state')
    need('<section id="databaseLoading" class="database-loading"' not in index,'1.4.3 loading panel reuses database-loading body state class')
    need(re.search(r'(?<![\w-])\.database-loading\s*\{',css) is None,'1.4.3 generic database-loading selector can restyle the body')
    need('body.startup-layout-hold #databaseLoading{position:fixed;z-index:590;left:50%;top:50%;right:auto;bottom:auto;transform:translate(-50%,-50%)' in css,'1.4.3 startup loading panel is not viewport-centered')
    need('const networkAddressReveal={active:false,ipv4:false,ipv6:false,pseudo:false};' in app,'1.4.3 independent network address reveal slots missing')
    need("networkAddressMarkup(n.activeAddress,activeFamily,'active')" in app and "networkAddressMarkup(n.ipv6.address,'IPv6','ipv6')" in app,'1.4.3 independent Active/IPv6 privacy controls missing')
    need('const networkAddressShieldIcon=' in app and 'class="network-address-privacy-mark"' in app,'1.4.3 address privacy shield presentation missing')
    need('.network-address-mosaic::before{' in css and '.network-address-mosaic::after{' in css and '@keyframes networkMosaicSheen' in css,'1.4.3 opaque mosaic presentation missing')
    need('.settings-info-row-address{grid-template-columns:minmax(0,1fr)!important' in css and '.settings-info-row-address .network-address-toggle{width:100%;max-width:100%;min-width:0' in css,'1.4.3 full-width address row layout missing')
    need('.network-address-text{max-width:none;overflow:visible;text-overflow:clip;white-space:normal;overflow-wrap:anywhere;word-break:break-all' in css,'1.4.3 network address text can still truncate')
    need('@media(max-width:520px)' in css and '@media(max-width:340px)' in css,'1.4.3 mobile address-fit breakpoints missing')
    need("document.body?.classList.toggle('database-loading',!!visible)" in app and 'body.database-loading #filters{display:none!important}' in css,'1.4.3 database-loading/filter hold missing')
    need('<body class="database-loading startup-layout-hold">' in index,'1.4.3 parse-time startup layout hold missing')
    need("body.classList.remove('startup-layout-hold');body.classList.add('startup-layout-ready')" in app,'1.4.3 startup layout release owner missing')
    ...
    need("'assets/app.v1402.js'" in text('tools/build_pages_artifact.py') and "'assets/app.v1403.js'" in text('tools/build_pages_artifact.py') and "'assets/app.v1400.js'" not in text('tools/build_pages_artifact.py'),'Pages immediate-predecessor/current app bridge mismatch')
    need("PREDECESSOR_BRIDGE = {'app.v1402.js','browser-compat.v1402.js','release-bootstrap.v1402.js'}" in text('tools/repair_repository.py'),'1.4.3 repository predecessor bridge mismatch')
    for name,wf in [('.github/workflows/pages.yml',workflow),('tools/pages.workflow.yml',workflow_template)]:
        need(wf.count('python tools/verify_1_4_3_startup_loader_address_fit.py')>=3,f'{name}: 1.4.3 verifier missing from release stages')
        need(wf.count('python tools/test_1_4_3_startup_loader_address_fit_negative_mutations.py')>=3,f'{name}: 1.4.3 negative suite missing from release stages')
    ...
    need(versioned==['app.v1402.js',APP_ASSET],f'exactly current + predecessor versioned frontend apps are permitted: {versioned}')
    ...
    if node:
        for rel in [f'assets/{APP_ASSET}','assets/browser-compat.v1403.js','assets/encyclopedia.v03924.js','worker/src/index.js','worker/tests/contract.mjs']:
