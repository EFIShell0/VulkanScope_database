...
def audit_artifact_tree(artifact_root: Path):
    ...
    allowed_assets={
        'app.v1005.js','encyclopedia.v03924.js','site.v0390.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png',
    ...
    if idx.is_file():
        ...
        acheck('app.v1005.js' in body and 'encyclopedia.v03924.js' in body and 'config.js?v=1005' in body and 'site.v0390.css' in body,'Pages artifact current asset references')
    ...
    if artifact_errors:
        print("\n".join(artifact_errors)); sys.exit(1)
    print('VulkanScope Database 1.0.5 Pages artifact audit: PASS')
    sys.exit(0)
...
app=text(root/'assets/app.v1005.js')
...
check('VulkanScope Database <strong>1.0.5</strong>' in index,'index version')
check('site.v0390.css' in index and 'app.v1005.js' in index and 'encyclopedia.v03924.js' in index and 'config.js?v=1005' in index,'1.0.5 cache-busted asset refs')
check('site.v0390.css?v=1005' in index and 'app.v1005.js?v=1005' in index,'1.0.5 UI cache key')
check('Database 1.0.5' in app,'frontend database version')
...
check('node --check assets/app.v1005.js' in workflow,'workflow frontend syntax check')
...
check(static.get('databaseVersion')=='1.0.5','static database version')
...
check('\"databaseVersion\":\"1.0.5\"' in build_index and '\"producerQueryBaseline\":\"VulkanScope 1.0.15 · Vulkan 1.4.362\"' in build_index,'build_index current release metadata')
...
check(pkg.get('version')=='1.0.5','worker package version')
check(pkg.get('devDependencies',{}).get('wrangler')=='4.130.0','Wrangler pin')
check(pkg.get('scripts',{}).get('security:audit')=='npm audit --audit-level=high','high/critical npm audit gate')
...
check(versioned_apps==['app.v1005.js'],f'exactly one versioned frontend app asset is permitted; run repository repair: {versioned_apps}')
...
if node:
    for f in [root/'assets/app.v1005.js',root/'assets/encyclopedia.v03924.js',root/'worker/src/index.js',root/'worker/tests/contract.mjs']:
