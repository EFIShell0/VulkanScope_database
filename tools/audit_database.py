...
def audit_artifact_tree(artifact_root: Path):
    ...
    allowed_assets={
        'app.v1006.js','encyclopedia.v03924.js','site.v0390.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png',
    ...
    if idx.is_file():
        ...
        acheck('app.v1006.js' in body and 'encyclopedia.v03924.js' in body and 'config.js?v=1006' in body and 'site.v0390.css' in body,'Pages artifact current asset references')
    ...
    if artifact_errors:
        print("\n".join(artifact_errors)); sys.exit(1)
    print('VulkanScope Database 1.0.6 Pages artifact audit: PASS')
    sys.exit(0)
...
app=text(root/'assets/app.v1006.js')
...
check('VulkanScope Database <strong>1.0.6</strong>' in index,'index version')
check('site.v0390.css' in index and 'app.v1006.js' in index and 'encyclopedia.v03924.js' in index and 'config.js?v=1006' in index,'1.0.6 cache-busted asset refs')
check('site.v0390.css?v=1006' in index and 'app.v1006.js?v=1006' in index,'1.0.6 UI cache key')
check('Database 1.0.6' in app,'frontend database version')
...
if (root/'assets/encyclopedia.v03924.js').is_file():
    encyclopedia=text(root/'assets/encyclopedia.v03924.js')
    for token in ['\"commands\":842','\"tokens\":6248','\"types\":2461','\"extensions\":476','\"vkResults\":50']:
...
check('node --check assets/app.v1006.js' in workflow,'workflow frontend syntax check')
...
check('python tools/audit_database.py --version' in workflow and 'python tools/repair_repository.py --check' in workflow and 'python tools/repair_repository.py --apply' in workflow,'workflow exposes repair/audit fingerprints')
...
check('python tools/verify_1_0_0_producer_baseline.py --skip-version' in workflow and 'python tools/verify_1_0_1_full_audit.py --skip-version' in workflow and 'node tools/test_surface_evidence_1001.mjs' in workflow and 'python tools/test_1_0_1_full_audit_negative_mutations.py' in workflow and 'python tools/verify_1_0_2_producer_baseline.py' in workflow and 'python tools/test_1_0_2_producer_baseline_state_machine.py' in workflow and 'python tools/test_1_0_2_producer_baseline_negative_mutations.py' in workflow,'workflow current 1.0.2 producer and retained 1.0.1 gates')
...
check('historicalPropertyFeatureCompareIdentity' in app and 'producerBefore04140' in app,'0.39.22 historical property/feature Compare identity')
check("const visibleMetricLabel=diff?'Visible differences':'Visible fields'" in app,'0.39.22 Compare visible metric semantics')
check("!['true','false'].includes(v.toLowerCase())" in app,'0.39.22 historical mapping is boolean-only')
check('surfacePresentationState' in app and 'surfaceQueueState' in app,'0.39.22 Surface evidence-state helpers')
check("put('SURFACE',k,canonicalSurfaceValue(k,v),availabilityStatus(v))" in app,'0.39.22 generic Surface values use availability semantics')
check("queryStatus:d.surface.queryStatus??'unknown'" in app,'0.39.22 structured Surface queryStatus preservation')
check('node tools/test_surface_compare_04142.mjs' in workflow,'workflow Surface Compare semantic test')
check('node tools/test_surface_compare_04142_negative_mutations.mjs' in workflow,'workflow Surface Compare negative-mutation test')
...
check("publishedVulkanSpec:'Vulkan 1.4.362 (2026-09-04)'" in worker,'published spec metadata')
check('VulkanScope producer/query baseline 1.4.362' in worker,'producer registry metadata')
check('VulkanScope 1.0.15 · Vulkan 1.4.362' in worker,'current producer metadata')
for token in ['producerVersion=p=>','producerAtLeast0803=p=>','supportedProducer=p=>','producerAtLeast0414=p=>','currentProducerIdentity=p=>','p.application.versionCode===800+v.patch','validSecurityPatch=p=>','applicationAbiConsistent=p=>','validCurrentQueueSemantics=p=>','validCurrentQueryDiagnostics=p=>','producerAtLeast0418=p=>','producerAtLeast0419=p=>','producerAtLeast04110=p=>','validCurrentImageFormatTupleSemantics=p=>']:
...
check("validation='envelope_shape'" in worker and "validation='producer_identity'" in worker and "'vulkan_1_4_362_registry_contract'" in worker,'0.39.27 precise current submission validation classes')
check("registryVersion=producerAtLeast08010(p)?'1.4.362':'1.4.361'" in worker and 'baseline=`Vulkan ${registryVersion}`' in worker,'0.39.27 exact producer registry-baseline prefix contract')
check('header=`${baseline} compile headers; validated query catalog ${baseline}`' in worker,'0.39.27 exact registry/header provenance derivation')
...
'Release 0.39.21 / VulkanScope 0.41.46 producer-baseline requirements', 'Release 0.39.22 / VulkanScope 0.80.8 loading, producer-floor, canonical-device-type and page-scroll requirements', 'Release 0.39.23 / VulkanScope 0.80.9 producer-floor and Vulkan Encyclopedia requirements', 'Release 0.39.24 / VulkanScope 0.80.10 / Vulkan 1.4.362 requirements', 'Release 0.39.25 loading / Encyclopedia UI coherence requirements','Release 0.39.26 / VulkanScope 0.80.12 submission-diagnostics requirements','Release 0.39.27 exact producer registry-baseline string requirements','Release 1.0.0 / VulkanScope 1.0.0 producer-baseline requirements','Release 1.0.1 full security, specification, correctness and Surface evidence requirements','Release 1.0.2 / VulkanScope 1.0.2 producer-baseline requirements']
for token in required_rules: check(token in rules,f'release rule {token}')
for rel in ['rules/0.37.0_VULKANSCOPE_0.41.0_TRENDS_PERMALINK_AUDIT.md','rules/0.37.1_QUEUE_VIDEO_QUERY_STATE_AUDIT.md','rules/0.38.0_STATISTICS_HASH_ROUTING_0.41.4_FULL_AUDIT.md','rules/0.39.0_FILTER_STATISTICS_FULL_AUDIT.md','rules/0.39.1_VULKANSCOPE_0.41.5_COMPATIBILITY_HARDENING.md','rules/0.39.2_CI_PAGES_ARTIFACT_HYGIENE.md','rules/0.39.3_GITHUB_ACTIONS_SOURCE_AUDIT_HARDENING.md','rules/0.39.4_TRACKED_SOURCE_AUDIT_REPOSITORY_REPAIR.md','rules/0.39.5_CROSS_PRODUCER_COMPARE_0.41.7_AUDIT.md','rules/0.39.6_IMAGE_FORMAT_PROPERTIES2_TUPLE_STATE_0.41.8_AUDIT.md','rules/0.39.7_IMAGE_FORMAT_QUERY_OUTCOME_SEPARATION_0.41.9_AUDIT.md','rules/0.39.8_IMAGE_FORMAT_QUERY_STATE_COMPLETENESS_0.41.10_AUDIT.md','rules/0.39.10_ASTC_CANONICAL_FORMAT_SUBMISSION_0.41.12_AUDIT.md','rules/0.39.11_QUERY_GROUP_UNAVAILABLE_SUBMISSION_0.41.12_AUDIT.md','rules/0.39.12_VULKAN_1.4.361_VULKANSCOPE_0.41.32_REGRESSION_RESOURCE_AUDIT.md','rules/0.39.13_WINDOWS_UTF8_QUALITY_GATE_AUDIT.md','rules/0.39.14_EXISTING_REPOSITORY_OVERLAY_REGRESSION_AUDIT.md','rules/0.39.15_VULKANSCOPE_0.41.40_REPORT_TEXT_COMPATIBILITY_AUDIT.md','rules/0.39.16_VULKANSCOPE_0.41.41_HISTORICAL_COMPARE_IDENTITY_AUDIT.md','rules/0.39.17_VULKANSCOPE_0.41.42_SURFACE_EVIDENCE_STATE_AUDIT.md','rules/0.39.18_VULKANSCOPE_0.41.43_PRODUCER_BASELINE_AUDIT.md','rules/0.39.20_VULKANSCOPE_0.41.45_PRODUCER_BASELINE_AUDIT.md','rules/0.39.21_VULKANSCOPE_0.41.46_PRODUCER_BASELINE_AUDIT.md','rules/0.39.22_VULKANSCOPE_0.80.8_LOADING_FLOOR_CANONICAL_TYPE_AUDIT.md','rules/0.39.23_VULKANSCOPE_0.80.9_FLOOR_ENCYCLOPEDIA_AUDIT.md','rules/0.39.24_VULKAN_1.4.362_VULKANSCOPE_0.80.10_AUDIT.md','rules/0.39.25_LOADING_ENCYCLOPEDIA_UI_COHERENCE_AUDIT.md','rules/0.39.26_VULKANSCOPE_0.80.12_SUBMISSION_DIAGNOSTICS_AUDIT.md','rules/0.39.27_VULKAN_REGISTRY_BASELINE_PREFIX_AUDIT.md','rules/1.0.0_VULKANSCOPE_1.0.0_PRODUCER_BASELINE_AUDIT.md','rules/1.0.1_FULL_SECURITY_SPEC_CORRECTNESS_USABILITY_AUDIT.md','rules/1.0.2_VULKANSCOPE_1.0.2_PRODUCER_BASELINE_AUDIT.md']:
...
check(static.get('databaseVersion')=='1.0.6','static database version')
...
check('\"databaseVersion\":\"1.0.6\"' in build_index and '\"producerQueryBaseline\":\"VulkanScope 1.0.15 · Vulkan 1.4.362\"' in build_index,'build_index current release metadata')
...
check(pkg.get('version')=='1.0.6','worker package version')
...
security_audit=text(root/'worker/scripts/security-audit.mjs')
check("run(['audit','--audit-level=high'])" in security_audit and "'--package-lock-only'" in security_audit,'security audit lock bootstrap/high-severity gate')
check('security:audit' in pkg.get('scripts',{}).get('predeploy',''),'deploy security audit guard')
check(all((pkg.get('allowScripts') or {}).get(k) is True for k in ['esbuild','sharp','workerd']),'reviewed npm install-script allowlist')
...
check(versioned_apps==['app.v1006.js'],f'exactly one versioned frontend app asset is permitted; run repository repair: {versioned_apps}')
...
if node:
    for f in [root/'assets/app.v1006.js',root/'assets/encyclopedia.v03924.js',root/'worker/src/index.js',root/'worker/tests/contract.mjs']:
