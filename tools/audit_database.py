...
def audit_artifact_tree(artifact_root: Path):
    ...
    allowed_assets={
        'app.v03911.js','site.v0390.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png',
    ...
    if idx.is_file():
        ...
        acheck('app.v03911.js' in body and 'config.js?v=03911' in body and 'site.v0390.css' in body,'Pages artifact current asset references')
    ...
    if artifact_errors:
        print("\n".join(artifact_errors)); sys.exit(1)
    print('VulkanScope Database 0.39.11 Pages artifact audit: PASS')
    sys.exit(0)
...
app=text(root/'assets/app.v03911.js')
...
check('VulkanScope Database <strong>0.39.11</strong>' in index,'index version')
check('site.v0390.css' in index and 'app.v03911.js' in index and 'config.js?v=03911' in index,'0.39.11 cache-busted asset refs')
check('Database 0.39.11' in app,'frontend database version')
check('VulkanScope 0.41.13 · Vulkan 1.4.360' in app,'frontend producer baseline')
check("connect-src 'self' https://vulkanscope-database-api.vulkanscope.workers.dev" in index,'CSP API pin')
check('node --check assets/app.v03911.js' in workflow,'workflow frontend syntax check')
...
for token in ['legacyProfiles=[]',"String(c.section||'')==='VULKAN PROFILES'","put('PROFILES',p.name",'Common evidence only','Cross-producer comparison:','Profile definition revisions differ:','Common fields','One-sided fields']:
    check(token in app,f'0.39.11 compare contract {token}')
...
check('VulkanScope 0.41.13 · Vulkan 1.4.360' in worker,'current producer metadata')
for token in ['producerVersion=p=>','supportedProducer=p=>','producerAtLeast0414=p=>','currentProducerIdentity=p=>','validSecurityPatch=p=>','applicationAbiConsistent=p=>',"p.application.version!=='0.41.5'||p.application.versionCode===415","p.application.version!=='0.41.6'||p.application.versionCode===416","p.application.version!=='0.41.7'||p.application.versionCode===417","p.application.version!=='0.41.8'||p.application.versionCode===418","p.application.version!=='0.41.9'||p.application.versionCode===419","p.application.version!=='0.41.10'||p.application.versionCode===420",'validCurrentQueueSemantics=p=>','validCurrentQueryDiagnostics=p=>','producerAtLeast0418=p=>','producerAtLeast0419=p=>','producerAtLeast04110=p=>','validCurrentImageFormatTupleSemantics=p=>']:
...
check(static.get('databaseVersion')=='0.39.11','static database version')
check(static.get('normalizerVersion')==16,'static normalizer')
check(static.get('producerQueryBaseline')=='VulkanScope 0.41.13 · Vulkan 1.4.360','static producer baseline')
build_index=text(root/'tools/build_index.py')
check('\"databaseVersion\":\"0.39.11\"' in build_index and '\"producerQueryBaseline\":\"VulkanScope 0.41.13 · Vulkan 1.4.360\"' in build_index,'build_index current release metadata')
...
check(pkg.get('version')=='0.39.11','worker package version')
...
check(versioned_apps==['app.v03911.js'],f'exactly one versioned frontend app asset is permitted; run repository repair: {versioned_apps}')
...
if node:
    for f in [root/'assets/app.v03911.js',root/'worker/src/index.js',root/'worker/tests/contract.mjs']:
