...
ASSET_ALLOW={
    'app.v1403.js','browser-compat.v1403.js','release-bootstrap.v1403.js',APP_ASSET,'browser-compat.v1404.js','release-bootstrap.v1404.js','encyclopedia.v03924.js','site.v0390.css','site.v1308.css','site.v1309.css','site.v1400.css','site.v1401.css','site.v1402.css','site.v1403.css','site.v1404.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png','favicon.ico','favicon.png','vulkanscope_logo_horizontal.png','browser-logos/google-chrome.v1307.svg','browser-logos/chromium.v1307.png','browser-logos/microsoft-edge.v1307.svg','browser-logos/firefox.v1307.svg','browser-logos/safari.v1307.svg','browser-logos/opera.v1307.svg','browser-logos/brave.v1307.svg','browser-logos/vivaldi.v1307.svg','browser-logos/samsung-browser.v1307.svg',
...
def audit_artifact(root:Path,require_release_ready=False):
    ...
    for token in [f'assets/{APP_ASSET}?v={CACHE_KEY}','site.v1404.css?v=1404',f'config.js?v={CACHE_KEY}',f'VulkanScope Database <strong>{DB_VERSION}</strong>']:
...
def audit_source(root:Path):
    ...
    for token in [f'VulkanScope Database <strong>{DB_VERSION}</strong>',f'assets/{APP_ASSET}?v={CACHE_KEY}','site.v1404.css?v=1404',f'config.js?v={CACHE_KEY}']:
    ...
    need('browser-compat.v1404.js?v=1404' in index and 'release-bootstrap.v1404.js?v=1404' in index and 'browserCompatibilityGate' in index,'browser compatibility/startup freshness references missing')
    ...
    boot=text('assets/release-bootstrap.v1404.js')
    ...
    need(worker.count("databaseReleaseVersion:'1.4.4'")>=3,'Worker databaseReleaseVersion mismatch')
    need(worker.count("workerReleaseVersion:'1.4.4'")>=3,'Worker workerReleaseVersion mismatch')
    ...
    need("'assets/app.v1403.js'" in text('tools/build_pages_artifact.py') and "'assets/app.v1404.js'" in text('tools/build_pages_artifact.py') and "'assets/app.v1402.js'" not in text('tools/build_pages_artifact.py'),'Pages immediate-predecessor/current app bridge mismatch')
    need("PREDECESSOR_BRIDGE = {'app.v1403.js','browser-compat.v1403.js','release-bootstrap.v1403.js'}" in text('tools/repair_repository.py'),'1.4.4 repository predecessor bridge mismatch')
    for name,wf in [('.github/workflows/pages.yml',workflow),('tools/pages.workflow.yml',workflow_template)]:
        need(wf.count('python tools/verify_1_4_4_viewport_scroll_progress.py')>=3,f'{name}: 1.4.4 verifier missing from release stages')
        need(wf.count('python tools/test_1_4_4_viewport_scroll_progress_negative_mutations.py')>=3,f'{name}: 1.4.4 negative suite missing from release stages')
    ...
    need(versioned==['app.v1403.js',APP_ASSET],f'exactly current + predecessor versioned frontend apps are permitted: {versioned}')
    ...
    if node:
        for rel in [f'assets/{APP_ASSET}','assets/browser-compat.v1404.js','assets/encyclopedia.v03924.js','worker/src/index.js','worker/tests/contract.mjs']:
