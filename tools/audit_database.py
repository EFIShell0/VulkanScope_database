...
def audit_artifact_tree(artifact_root: Path):
    ...
    allowed_assets={
        'app.v03927.js','encyclopedia.v03924.js','site.v0390.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png',
    ...
    if idx.is_file():
        ...
        acheck('app.v03927.js' in body and 'encyclopedia.v03924.js' in body and 'config.js?v=03927' in body and 'site.v0390.css' in body,'Pages artifact current asset references')
    ...
    if artifact_errors:
        print("\n".join(artifact_errors)); sys.exit(1)
    print('VulkanScope Database 0.39.27 Pages artifact audit: PASS')
    sys.exit(0)
...
app=text(root/'assets/app.v03927.js')
...
check('VulkanScope Database <strong>0.39.27</strong>' in index,'index version')
check('site.v0390.css' in index and 'app.v03927.js' in index and 'encyclopedia.v03924.js' in index and 'config.js?v=03927' in index,'0.39.27 cache-busted asset refs')
check('site.v0390.css?v=03927' in index and 'app.v03927.js?v=03927' in index,'0.39.27 UI hotfix cache key')
check('Database 0.39.27' in app,'frontend database version')
check('VulkanScope 0.80.12 · Vulkan 1.4.362' in app,'frontend producer baseline')
...
check('node --check assets/app.v03927.js' in workflow,'workflow frontend syntax check')
...
check('VulkanScope 0.80.12 · Vulkan 1.4.362' in worker,'current producer metadata')
...
check(static.get('databaseVersion')=='0.39.27','static database version')
check(static.get('normalizerVersion')==16,'static normalizer')
check(static.get('producerQueryBaseline')=='VulkanScope 0.80.12 · Vulkan 1.4.362','static producer baseline')
build_index=text(root/'tools/build_index.py')
check('\"databaseVersion\":\"0.39.27\"' in build_index and '\"producerQueryBaseline\":\"VulkanScope 0.80.12 · Vulkan 1.4.362\"' in build_index,'build_index current release metadata')
...
check(pkg.get('version')=='0.39.27','worker package version')
...
check(versioned_apps==['app.v03927.js'],f'exactly one versioned frontend app asset is permitted; run repository repair: {versioned_apps}')
...
if node:
    for f in [root/'assets/app.v03927.js',root/'assets/encyclopedia.v03924.js',root/'worker/src/index.js',root/'worker/tests/contract.mjs']:
