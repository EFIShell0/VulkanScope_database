...
app=(root/'assets/app.v0360.js').read_text()
worker=(root/'worker/src/index.js').read_text()
rules=(root/'rules/PROJECT_RULES.md').read_text()
check('VulkanScope Database <strong>0.36.0</strong>' in index,'index version')
check('site.v0360.css' in index and 'app.v0360.js' in index and 'config.js?v=0360' in index,'cache-busted asset refs')
check("connect-src 'self' https://vulkanscope-database-api.vulkanscope.workers.dev" in index,'CSP API pin')
check('Database 0.36.0' in app,'frontend version')
...
check('Release 0.35.9 Display & HDR / ABI / home-metric parity' in rules and 'Release 0.36.0 luminance-unit typography parity' in rules,'release rules')
...
print('VulkanScope Database 0.36.0 audit: PASS')
