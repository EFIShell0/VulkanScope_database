...
AUDIT_VERSION='1.4.7'
DB_VERSION='1.4.7'
APP_ASSET='app.v1407.js'
CACHE_KEY='1407'
PRODUCER='VulkanScope 1.4.3 · Vulkan 1.4.362'
...
ASSET_ALLOW={
    'app.v1406.js','browser-compat.v1406.js','release-bootstrap.v1406.js',APP_ASSET,'browser-compat.v1407.js','release-bootstrap.v1407.js','encyclopedia.v03924.js','site.v0390.css','site.v1308.css','site.v1309.css','site.v1400.css','site.v1401.css','site.v1402.css','site.v1403.css','site.v1404.css','site.v1405.css','site.v1406.css','site.v1407.css','apple-touch-icon-v0311.png','favicon-v0311.ico','favicon-v0311.png','favicon.ico','favicon.png','vulkanscope_logo_horizontal.png','browser-logos/google-chrome.v1307.svg','browser-logos/chromium.v1307.png','browser-logos/microsoft-edge.v1307.svg','browser-logos/firefox.v1307.svg','browser-logos/safari.v1307.svg','browser-logos/opera.v1307.svg','browser-logos/brave.v1307.svg','browser-logos/vivaldi.v1307.svg','browser-logos/samsung-browser.v1307.svg',
...
def audit_artifact(root:Path,require_release_ready=False):
    ...
    for token in [f'assets/{APP_ASSET}?v={CACHE_KEY}','site.v1407.css?v=1407',f'config.js?v={CACHE_KEY}',f'VulkanScope Database <strong>{DB_VERSION}</strong>']:
...
def audit_source(root:Path):
    ...
    for token in [f'VulkanScope Database <strong>{DB_VERSION}</strong>',f'assets/{APP_ASSET}?v={CACHE_KEY}','site.v1407.css?v=1407',f'config.js?v={CACHE_KEY}']:
    ...
    need('browser-compat.v1407.js?v=1407' in index and 'release-bootstrap.v1407.js?v=1407' in index and 'browserCompatibilityGate' in index,'browser compatibility/startup freshness references missing')
    ...
    boot=text('assets/release-bootstrap.v1407.js')
    ...
    need("go.disabled=single||target===value" in app and "go.disabled=max<=1||!valid||target===currentPage()" in app,'page Go current-state disabling missing')
    ...
    need("const browserLanguage=()=>{const raw=String(navigator.languages?.[0]||navigator.language" in app and "new Intl.DisplayNames(['en'],{type:'region'})" in app,'browser/system regional presentation contract missing')
    ...
    need(worker.count("databaseReleaseVersion:'1.4.7'")>=3,'Worker databaseReleaseVersion mismatch')
    need(worker.count("workerReleaseVersion:'1.4.7'")>=3,'Worker workerReleaseVersion mismatch')
    ...
    for token in [SPEC,'VulkanScope producer/query baseline 1.4.362',PRODUCER,'VulkanScope 1.4.3+ · schema 2 / technical report 3']:
    ...
    need("producerAtLeast1403" in worker and "versionAtLeast(v,1,4,3)" in worker and "VulkanScope 1.4.3 or newer is required for new submissions" in worker,'VulkanScope 1.4.3 POST floor missing')
    ...
    need('const BROWSER_LOGO_ASSETS={' not in app and 'browserLogoAsset(' not in app and 'class=\"settings-browser-icon\"' not in app and 'class=\"settings-browser-logo\"' not in app,'1.3.10 Browser information must preserve the 1.3.9 text-only contract')
    need('settings-browser-summary settings-browser-summary-text' in app and 'No browser logo, icon, favicon service or browser-brand image is displayed or loaded.' in app,'1.3.10 browser text-only disclosure missing')
    need('assets/browser-logos/' not in app and 'browser-logos' not in index,'1.3.10 current frontend unexpectedly references browser marks')
    for rel in ['google-chrome.v1307.svg','chromium.v1307.png','microsoft-edge.v1307.svg','firefox.v1307.svg','safari.v1307.svg','opera.v1307.svg','brave.v1307.svg','vivaldi.v1307.svg','samsung-browser.v1307.svg']:
        need((root/'assets/browser-logos'/rel).is_file(),f'missing immutable historical browser-mark asset {rel}')
    need('## Release 1.3.8 filter / regional-mode / browser-cleanup requirements' in rules and '## Release 1.3.9 regional-control fit / country-profile / filter-gutter requirements' in rules and '## Release 1.3.10 country-selector search / pagination requirements' in rules and '## Release 1.4.0 submission floor / Settings order / address mosaic / startup-filter requirements' in rules,'1.3.8/1.3.9/1.3.10/1.4.0 rules sections missing')
    need('id=\"settingsRegionalMode\"' in index and "REGIONAL_MODES=new Set(['auto','country','manual'])" in app,'1.3.9 regional mode UI/state missing')
    need("const enabled=mode==='country'?id==='settingsRegionalCountry':mode==='manual'?id!=='settingsRegionalCountry':false" in app,'1.3.9 Automatic/Country/Manual lock matrix missing')
    need("else if(state.regionalMode==='manual')state.regionalCountry='auto'" in app,'1.3.9 Manual mode must clear country override')
    need("if(mode==='auto'||mode==='manual')return browserLanguage();" in app,'1.3.9 Manual mode must use browser/system region')
    need('const COUNTRY_PRIMARY_TIME_ZONES=Object.freeze(' in app and 'const countryRegionalProfile=(' in app,'1.3.9 country-derived regional profile contract missing')
    for label in ['Date format','Clock format','Representative IANA zone','Current UTC offset','Standard / winter offset','Daylight / summer offset','Seasonal clock change','Current offset state']:
        need(label in app,f'1.3.9 country profile row missing: {label}')
    need("host.classList.toggle('has-surface-scrollbar',scrollable)" in app and "parent.classList.toggle('has-surface-scrollbar',scrollable)" in app,'1.3.9 demand-driven scrollbar host state missing')
    need('.custom-select-scroll{padding-right:0!important}' in css and '.custom-select-scroll.has-surface-scrollbar{padding-right:23px!important}' in css,'1.3.9 demand-driven filter gutter CSS missing')
    need('.settings-regional-group .settings-select-row{grid-template-columns:minmax(0,1fr)!important' in css and '.settings-regional-choice .filter-choice-label' in css,'1.3.9 full-width regional control fit CSS missing')
    # 1.3.10 searchable/paged regional large selectors.
    need("const SETTINGS_SEARCHABLE_SELECTS=new Set(['settingsRegionalCountry','settingsRegionalTimeZone'])" in app,'1.3.10 regional large-selector searchable allow-list missing')
    need("sel.id==='settingsRegionalCountry'?'Search country / region':sel.id==='settingsRegionalTimeZone'?'Search time zones'" in app,'1.3.10 country/time-zone search labels missing')
    need('const CUSTOM_SELECT_OPTION_LIMIT=50;' in app and 'optionPages=Math.max(1,Math.ceil(matches.length/CUSTOM_SELECT_OPTION_LIMIT))' in app and 'visible=matches.slice(start,start+CUSTOM_SELECT_OPTION_LIMIT)' in app,'1.3.10 deterministic 50-option selector pagination missing')
    need('pager.hidden=!searchable' in app and "pageJumpMarkup(1,1,'custom-select-page-jump')" in app,'1.3.10 searchable selector pager wiring missing')
    need("if(searchable)search.addEventListener('input',()=>{optionPage=1;renderOptions({resetScroll:true})})" in app,'1.3.10 selector search must reset to page 1')
    need('optionPage=1;renderOptions({resetScroll:true,ensureSelected:!customSelectBrowseFromFirstPage(sel)})' in app,'1.3.10 Country/Time-zone menus must open from the first browse page instead of the selected-value page')
    need("items=COUNTRY_CODES.map(code=>[code,display?.of(code)||code]).sort((a,b)=>naturalCollator.compare(a[1],b[1]))" in app,'1.3.10 country options must remain alphabetically sorted by English display name')
    # 1.4.0 submission floor, Settings category order, address mosaic and startup filter flash prevention.
    need('id="filters" hidden' in index,'1.4.0 startup filter panel must be hidden until the first rendered view')
    ...
    need('## Release 1.4.4 viewport-scroll / page-progress containing-block requirements' in rules,'1.4.4 rules section missing')
    need('<section id="databaseLoading" class="database-loading-panel" role="status" aria-live="polite" aria-atomic="true">' in index,'1.4.4 loading panel must use a presentation class distinct from the body state')
    need('<section id="databaseLoading" class="database-loading"' not in index,'1.4.4 loading panel reuses database-loading body state class')
    need(re.search(r'(?<![\w-])\.database-loading\s*\{',css) is None,'1.4.4 generic database-loading selector can restyle the body')
    need('body.startup-layout-hold #databaseLoading{position:fixed;z-index:590;left:50%;top:50%;right:auto;bottom:auto;transform:translate(-50%,-50%)' in css,'1.4.4 startup loading panel is not viewport-centered')
    need('const networkAddressReveal={active:false,ipv4:false,ipv6:false,pseudo:false};' in app,'1.4.4 independent network address reveal slots missing')
    need("networkAddressMarkup(n.activeAddress,activeFamily,'active')" in app and "networkAddressMarkup(n.ipv6.address,'IPv6','ipv6')" in app,'1.4.4 independent Active/IPv6 privacy controls missing')
    need('const networkAddressShieldIcon=' in app and 'class="network-address-privacy-mark"' in app,'1.4.4 address privacy shield presentation missing')
    need('.network-address-mosaic::before{' in css and '.network-address-mosaic::after{' in css and '@keyframes networkMosaicSheen' in css,'1.4.4 opaque mosaic presentation missing')
    need('.settings-info-row-address{grid-template-columns:minmax(0,1fr)!important' in css and '.settings-info-row-address .network-address-toggle{width:100%;max-width:100%;min-width:0' in css,'1.4.4 full-width address row layout missing')
    need('.network-address-text{max-width:none;overflow:visible;text-overflow:clip;white-space:normal;overflow-wrap:anywhere;word-break:break-all' in css,'1.4.4 network address text can still truncate')
    need('@media(max-width:520px)' in css and '@media(max-width:340px)' in css,'1.4.4 mobile address-fit breakpoints missing')
    need("document.body?.classList.toggle('database-loading',!!visible)" in app and 'body.database-loading #filters{display:none!important}' in css,'1.4.4 database-loading/filter hold missing')
    need('<body class="database-loading startup-layout-hold">' in index,'1.4.4 parse-time startup layout hold missing')
    need("body.classList.remove('startup-layout-hold');body.classList.add('startup-layout-ready')" in app,'1.4.4 startup layout release owner missing')
    load_start=app.find('async function load(){'); first_render=app.find('render();',load_start); layout_release=app.find('finishStartupLayout()',load_start)
    need(load_start>=0 and first_render>load_start and layout_release>first_render,'1.4.4 startup layout is released before first report-backed render')
    need('body.startup-layout-hold #networkStatusShell' in css and 'body.startup-layout-hold .topbar' in css and 'body.startup-layout-hold .hero' in css and 'body.startup-layout-hold #contentView' in css and 'body.startup-layout-hold footer{display:none!important}' in css,'1.4.4 unstable startup shell suppression missing')
    need('@keyframes startupShellReveal{from{opacity:0}to{opacity:1}}' in css,'1.4.4 appRoot reveal must be opacity-only so fixed scroll chrome remains viewport-relative')
    need('.page-progress{position:fixed;inset:0 0 auto 0;height:3px;z-index:230;' in css,'1.4.4 page progress is not viewport-fixed above sticky header')
    need('.viewport-scrollbar{position:fixed;z-index:220;top:0;right:0;bottom:0;width:18px;' in css,'1.4.4 viewport scrollbar is not viewport-fixed')

    # 1.4.5 action-only address reveal + filter-search clear controls.
    need('## Release 1.4.5 action-only address reveal / filter-search clear requirements' in rules,'1.4.5 rules section missing')
    need('## Release 1.4.7 producer-floor metadata coherence requirements' in rules,'1.4.7 producer-floor metadata-coherence rules section missing')
    need('return`<span class="network-address-toggle ${revealed?' in app,'1.4.5 address shell is still interactive')
    need('<button class="network-address-toggle' not in app,'1.4.5 entire address shell remains a button')
    need('<button class="network-address-action" type="button" data-network-address-toggle="${esc(key)}"' in app,'1.4.5 dedicated Show/Hide button missing')
    need("const shell=button.closest('.network-address-toggle');shell?.classList.toggle('is-revealed',!!revealed)" in app,'1.4.5 address action does not update containing privacy shell')
    need('.network-address-toggle{cursor:default}' in css and '.network-address-action{appearance:none;-webkit-appearance:none;border:1px solid transparent;cursor:pointer' in css,'1.4.5 address click ownership CSS missing')
    need("clearSearch.className='search-clear-button custom-select-search-clear is-hidden'" in app,'1.4.5 custom filter X clear control missing')
    need("const syncSearchClear=()=>{const hidden=!search.value;" in app and "search.addEventListener('input',syncSearchClear);syncSearchClear();menu.appendChild(searchShell)" in app,'1.4.5 filter clear visibility synchronization missing')
    need("if(!search.value)return;search.value='';syncSearchClear();optionPage=1;renderOptions({resetScroll:true});search.focus({preventScroll:true})" in app,'1.4.5 filter clear atomic reset/focus behavior missing')
    need('.custom-select-search-clear{width:30px;flex:0 0 30px' in css and '.custom-select-search-clear.is-hidden{width:0;flex-basis:0' in css,'1.4.5 custom filter clear motion geometry missing')
    ...
    need("'assets/app.v1406.js'" in text('tools/build_pages_artifact.py') and "'assets/app.v1407.js'" in text('tools/build_pages_artifact.py') and "'assets/app.v1404.js'" not in text('tools/build_pages_artifact.py'),'Pages immediate-predecessor/current app bridge mismatch')
    need("PREDECESSOR_BRIDGE = {'app.v1406.js','browser-compat.v1406.js','release-bootstrap.v1406.js'}" in text('tools/repair_repository.py'),'1.4.7 repository predecessor bridge mismatch')
    for name,wf in [('.github/workflows/pages.yml',workflow),('tools/pages.workflow.yml',workflow_template)]:
        need(wf.count('python tools/verify_1_4_7_floor_metadata_coherence.py')>=3,f'{name}: 1.4.7 verifier missing from release stages')
        need(wf.count('python tools/test_1_4_7_floor_metadata_coherence_negative_mutations.py')>=3,f'{name}: 1.4.7 negative suite missing from release stages')
    ...
    need(versioned==['app.v1406.js',APP_ASSET],f'exactly current + predecessor versioned frontend apps are permitted: {versioned}')
    ...
    if node:
        for rel in [f'assets/{APP_ASSET}','assets/browser-compat.v1407.js','assets/encyclopedia.v03924.js','worker/src/index.js','worker/tests/contract.mjs']:
