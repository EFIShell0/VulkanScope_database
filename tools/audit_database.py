...
app=(root/'assets/app.v0364.js').read_text(encoding='utf-8')
worker=(root/'worker/src/index.js').read_text(encoding='utf-8')
rules=(root/'rules/PROJECT_RULES.md').read_text(encoding='utf-8')
check('VulkanScope Database <strong>0.36.5</strong>' in index,'index version')
check('site.v0362.css' in index and 'app.v0364.js' in index and 'config.js?v=0364' in index,'cache-busted asset refs')
check("connect-src 'self' https://vulkanscope-database-api.vulkanscope.workers.dev" in index,'CSP API pin')
check('Database 0.36.5' in app,'frontend version')
check('VulkanScope 0.35.1 · Vulkan 1.4.360' in app,'frontend producer baseline')
...
check('VulkanScope 0.35.1 · Vulkan 1.4.360' in worker,'current producer metadata')
for token in ['producerVersion=p=>','supportedProducer=p=>','currentProducerIdentity=p=>','validSecurityPatch=p=>','applicationAbiConsistent=p=>',"p.application.version!=='0.35.1'||p.application.versionCode===352"]:
...
for token in ['Release 0.35.9 Display & HDR / ABI / home-metric parity','Release 0.36.0 luminance-unit typography parity','Release 0.36.2 VulkanScope 0.34.2 complete-report parity','Release 0.36.3 technical-differences compare filter','Release 0.36.5 VulkanScope 0.35.1 complete-report contract']:
    check(token in rules,f'release rule {token}')
check((root/'rules/0.36.5_VULKANSCOPE_0.35.1_FULL_DATABASE_AUDIT.md').is_file(),'release audit document')
...
check(static.get('databaseVersion')=='0.36.5','static database version')
check(static.get('normalizerVersion')==13,'static normalizer')
check(static.get('producerQueryBaseline')=='VulkanScope 0.35.1 · Vulkan 1.4.360','static producer baseline')
workflow=(root/'.github/workflows/pages.yml').read_text(encoding='utf-8')
check('node --check assets/app.v0364.js' in workflow,'workflow frontend check')
...
check(pkg.get('version')=='0.36.5','worker package version')
...
if node:
    for f in [root/'assets/app.v0364.js',root/'worker/src/index.js',root/'worker/tests/contract.mjs']:
...
print('VulkanScope Database 0.36.5 audit: PASS')
