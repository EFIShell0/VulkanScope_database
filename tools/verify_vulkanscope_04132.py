...
w=(root/'worker/src/index.js').read_text(encoding='utf-8'); schema=json.loads((root/'report.schema.json').read_text(encoding='utf-8')); app=(root/'assets/app.v03927.js').read_text(encoding='utf-8'); errors=[]
def check(v,m):
...
for token in ["producerExactly04132",c['vulkanRegistryBaseline'],c['requiredPhysicalDeviceStruct'],f"r.catalogSchemaVersion!=={c['catalogSchemaVersion']}",f"r.implementedPhysicalDeviceStructCount!=={c['implementedPhysicalDeviceStructCount']}",f"r.validatedRuntimeQueryGroupCount!=={c['validatedRuntimeQueryGroupCount']}",f"r.runtimeRegistryTokenReferenceCount!=={c['runtimeRegistryTokenReferenceCount']}"]:
    check(str(token) in w,f'worker current producer contract missing {token}')
check(c['headerBaseline'] in w or ("registryVersion=producerAtLeast08010(p)?'1.4.362':'1.4.361'" in w and 'baseline=`Vulkan ${registryVersion}`' in w and 'header=`${baseline} compile headers; validated query catalog ${baseline}`' in w),f"worker current producer contract missing {c['headerBaseline']}")
