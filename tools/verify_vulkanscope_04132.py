...
w=(root/'worker/src/index.js').read_text(encoding='utf-8'); schema=json.loads((root/'report.schema.json').read_text(encoding='utf-8')); app=(root/'assets/app.v03924.js').read_text(encoding='utf-8'); errors=[]
def check(v,m):
...
check(c['headerBaseline'] in w or ("producerAtLeast08010(p)?'1.4.362':'1.4.361'" in w and 'Vulkan ${baseline} compile headers; validated query catalog Vulkan ${baseline}' in w),f"worker current producer contract missing {c['headerBaseline']}")
