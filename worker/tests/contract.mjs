...
assert.equal(j.databaseReleaseVersion,'1.2.10');
assert.equal(j.workerReleaseVersion,'1.2.10');
...
assert.match(j.producerQueryBaseline,/1\.2\.5/);
assert.match(j.compatibleProducer,/1\.2\.5\+/);
...
current.application.versionCode=1205;
...
below.application.version='1.2.4';
below.application.versionCode=1204;
below.reportText=reportText(below);
r=await call('/v1/reports',{method:'POST',body:below});
assert.equal(r.status,400,'VulkanScope 1.2.4 must be rejected by the 1.2.5 floor');
assert.match(await r.text(),/1\.2\.5 or newer/);

const oldMajor=structuredClone(current);
oldMajor.application.version='1.0.19';
oldMajor.application.versionCode=1019;
oldMajor.reportText=reportText(oldMajor);
r=await call('/v1/reports',{method:'POST',body:oldMajor});
assert.equal(r.status,400,'every producer below 1.2.5 must be rejected before generic validation');
assert.match(await r.text(),/1\.2\.5 or newer/);

const badIdentity=structuredClone(current);
badIdentity.application.versionCode=1204;
...
const malformedCurrent={application:{name:'VulkanScope',version:'1.2.5',versionCode:1205}};
...
assert.equal(compact.application.version,'1.2.5');
...
console.log('PASS Worker 1.2.10 transport contract: live sync head + VulkanScope 1.2.5 producer floor + historical reads + transport/security basics');
