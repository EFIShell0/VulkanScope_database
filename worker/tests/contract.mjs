...
let r=await call('/v1/health');assert.equal(r.status,200);let j=await r.json();assert.equal(j.normalizerVersion,16);assert.match(j.publishedVulkanSpec,/1\.4\.361/);assert.match(j.producerQueryBaseline,/0\.41\.40/);
r=await call('/v1/reports',{method:'POST',body:fixture()});assert.equal(r.status,201,await r.text());
let currentSplit=fixture();currentSplit.application.version='0.41.40';currentSplit.application.versionCode=450;currentSplit.reportText=reportText(currentSplit);r=await call('/v1/reports',{method:'POST',body:currentSplit});assert.equal(r.status,201,'0.41.40 split Loader API / Base probe instance API identity must be accepted');
