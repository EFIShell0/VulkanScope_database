...
assert.equal(j.databaseReleaseVersion,'1.4.1');
assert.equal(j.workerReleaseVersion,'1.4.1');
...
assert.match(j.producerQueryBaseline,/1\.4\.0/);
assert.match(j.compatibleProducer,/1\.4\.0\+/);
...
current.application.versionCode=1400;
...
below.application.version='1.3.13';
below.application.versionCode=1313;
below.reportText=reportText(below);
r=await call('/v1/reports',{method:'POST',body:below});
assert.equal(r.status,400,'VulkanScope 1.3.13 must be rejected by the 1.4.0 floor');
assert.match(await r.text(),/1\.4\.0 or newer/);
...
assert.equal(r.status,400,'every producer below 1.4.0 must be rejected before generic validation');
assert.match(await r.text(),/1\.4\.0 or newer/);
...
const malformedCurrent={application:{name:'VulkanScope',version:'1.4.0',versionCode:1400}};
...
assert.equal(compact.application.version,'1.4.0');
...
console.log('PASS Worker 1.4.1 transport contract: live sync head + VulkanScope 1.4.0 producer floor + historical reads + transport/security basics');
