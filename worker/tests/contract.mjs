...
let r=await call('/v1/health');assert.equal(r.status,200);let j=await r.json();assert.equal(j.normalizerVersion,16);assert.match(j.publishedVulkanSpec,/1\.4\.362/);assert.match(j.producerQueryBaseline,/1\.0\.15/);assert.match(j.compatibleProducer,/0\.80\.3\+/);
r=await call('/v1/reports',{method:'POST',body:fixture()});assert.equal(r.status,201,await r.text());
let current0810=fixture();current0810.application.version='0.80.10';current0810.application.versionCode=810;current0810.vulkan.registryBaseline='Vulkan 1.4.362';current0810.vulkan.headerBaseline='Vulkan 1.4.362 compile headers; validated query catalog Vulkan 1.4.362';current0810.technicalReport.registryCoverage.baseline='Vulkan 1.4.362';current0810.technicalReport.registryCoverage.headerBaseline=current0810.vulkan.headerBaseline;current0810.reportText=reportText(current0810);r=await call('/v1/reports',{method:'POST',body:current0810});assert.equal(r.status,201,'VulkanScope 0.80.10 / Vulkan 1.4.362 must be accepted');
let current0812=structuredClone(current0810);current0812.application.version='0.80.12';current0812.application.versionCode=812;current0812.reportText=reportText(current0812);r=await call('/v1/reports',{method:'POST',body:current0812});assert.equal(r.status,201,'VulkanScope 0.80.12 / 812 must be accepted');
let current1000=structuredClone(current0812);current1000.application.version='1.0.0';current1000.application.versionCode=1000;current1000.reportText=reportText(current1000);r=await call('/v1/reports',{method:'POST',body:current1000});assert.equal(r.status,201,'VulkanScope 1.0.0 / 1000 must be accepted');
let bad1000Identity=structuredClone(current1000);bad1000Identity.application.versionCode=816;bad1000Identity.reportText=reportText(bad1000Identity);r=await call('/v1/reports',{method:'POST',body:bad1000Identity});assert.equal(r.status,400);assert.match(await r.text(),/producer_identity/);
let current1001=structuredClone(current1000);current1001.application.version='1.0.1';current1001.application.versionCode=1001;current1001.reportText=reportText(current1001);r=await call('/v1/reports',{method:'POST',body:current1001});assert.equal(r.status,201,'VulkanScope 1.0.1 / 1001 must be accepted');
let bad1001Identity=structuredClone(current1001);bad1001Identity.application.versionCode=1000;bad1001Identity.reportText=reportText(bad1001Identity);r=await call('/v1/reports',{method:'POST',body:bad1001Identity});assert.equal(r.status,400);assert.match(await r.text(),/producer_identity/);
let current1002=structuredClone(current1001);current1002.application.version='1.0.2';current1002.application.versionCode=1002;current1002.reportText=reportText(current1002);r=await call('/v1/reports',{method:'POST',body:current1002});assert.equal(r.status,201,'VulkanScope 1.0.2 / 1002 must be accepted');
let bad1002Identity=structuredClone(current1002);bad1002Identity.application.versionCode=1001;bad1002Identity.reportText=reportText(bad1002Identity);r=await call('/v1/reports',{method:'POST',body:bad1002Identity});assert.equal(r.status,400);assert.match(await r.text(),/producer_identity/);
let current1015=structuredClone(current1002);current1015.application.version='1.0.15';current1015.application.versionCode=1015;current1015.reportText=reportText(current1015);r=await call('/v1/reports',{method:'POST',body:current1015});assert.equal(r.status,201,'VulkanScope 1.0.15 / 1015 must be accepted');
let bad1015Identity=structuredClone(current1015);bad1015Identity.application.versionCode=1014;bad1015Identity.reportText=reportText(bad1015Identity);r=await call('/v1/reports',{method:'POST',body:bad1015Identity});assert.equal(r.status,400);assert.match(await r.text(),/producer_identity/);
let unprefixed0812=structuredClone(current0812);unprefixed0812.vulkan.registryBaseline='1.4.362';unprefixed0812.technicalReport.registryCoverage.baseline='1.4.362';unprefixed0812.reportText=reportText(unprefixed0812);r=await call('/v1/reports',{method:'POST',body:unprefixed0812});assert.equal(r.status,400,'0.80.12 unprefixed registry baseline is not the producer contract');assert.match(await r.text(),/vulkan_1_4_362_registry_contract/);
let unprefixed0809=fixture();unprefixed0809.vulkan.registryBaseline='1.4.361';unprefixed0809.technicalReport.registryCoverage.baseline='1.4.361';unprefixed0809.reportText=reportText(unprefixed0809);r=await call('/v1/reports',{method:'POST',body:unprefixed0809});assert.equal(r.status,400,'0.80.9 unprefixed registry baseline is not the producer contract');assert.match(await r.text(),/vulkan_1_4_361_registry_contract/);
let bad0812Identity=structuredClone(current0812);bad0812Identity.application.versionCode=810;bad0812Identity.reportText=reportText(bad0812Identity);r=await call('/v1/reports',{method:'POST',body:bad0812Identity});assert.equal(r.status,400);assert.match(await r.text(),/producer_identity/);
let bad0812Registry=structuredClone(current0812);bad0812Registry.technicalReport.registryCoverage.implementedPhysicalDeviceStructCount=109;bad0812Registry.reportText=reportText(bad0812Registry);r=await call('/v1/reports',{method:'POST',body:bad0812Registry});assert.equal(r.status,400);assert.match(await r.text(),/vulkan_1_4_362_registry_contract/);
let bad0812Envelope=structuredClone(current0812);bad0812Envelope.application.unexpected='x';bad0812Envelope.reportText=reportText(bad0812Envelope);r=await call('/v1/reports',{method:'POST',body:bad0812Envelope});assert.equal(r.status,400);assert.match(await r.text(),/envelope_shape/);
let current0810OldRegistry=structuredClone(current0810);current0810OldRegistry.vulkan.registryBaseline='Vulkan 1.4.361';current0810OldRegistry.vulkan.headerBaseline='Vulkan 1.4.361 compile headers; validated query catalog Vulkan 1.4.361';current0810OldRegistry.technicalReport.registryCoverage.baseline='Vulkan 1.4.361';current0810OldRegistry.technicalReport.registryCoverage.headerBaseline=current0810OldRegistry.vulkan.headerBaseline;current0810OldRegistry.reportText=reportText(current0810OldRegistry);r=await call('/v1/reports',{method:'POST',body:current0810OldRegistry});assert.equal(r.status,400,'VulkanScope 0.80.10 must not claim the older Vulkan 1.4.361 collector baseline');
let minimumProducer=fixture();minimumProducer.application.version='0.80.3';minimumProducer.application.versionCode=803;minimumProducer.reportText=reportText(minimumProducer);r=await call('/v1/reports',{method:'POST',body:minimumProducer});assert.equal(r.status,201,'0.80.3 is the minimum accepted new producer');
let below0803=fixture();below0803.application.version='0.80.2';below0803.application.versionCode=802;below0803.reportText=reportText(below0803);r=await call('/v1/reports',{method:'POST',body:below0803});assert.equal(r.status,400,'0.80.2 must be rejected by the producer floor');assert.match(await r.text(),/0\.80\.3 or newer/);
let currentSplit=fixture();currentSplit.application.version='0.41.41';currentSplit.application.versionCode=451;currentSplit.reportText=reportText(currentSplit);r=await call('/v1/reports',{method:'POST',body:currentSplit});assert.equal(r.status,400,'producers below 0.80.1 must be rejected');
...
r=await call(`/v1/reports/${accepted04132.id}?compact=1`);assert.equal(r.status,200);let compact=await r.json();assert.equal(compact.application.version,'0.80.9');assert.equal(compact.capabilities,undefined,'compact detail must not duplicate normalized capability arrays');assert.equal(compact.technicalReport.registryCoverage.baseline,'Vulkan 1.4.361');
...
const historicalId='a'.repeat(64),historicalPayload=fixture();historicalPayload.application.version='0.41.46';historicalPayload.application.versionCode=456;historicalPayload.gpu.deviceType='Integrated GPU';historicalPayload.technicalReport.devices[0].deviceType='Integrated GPU';env.DB.rows.set(historicalId,{id:historicalId,submitted_at:'2026-01-01T00:00:00.000Z',payload_json:JSON.stringify(historicalPayload)});r=await call(`/v1/reports/${historicalId}?compact=1`);assert.equal(r.status,200);const historicalRead=await r.json();assert.equal(historicalRead.gpu.deviceType,'VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU','historical Integrated GPU must be canonicalized on read');assert.equal(historicalRead.technicalReport.devices[0].deviceType,'VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU');
...
let historical04110=fixture();historical04110.application.version='0.41.10';historical04110.application.versionCode=420;historical04110.reportText=reportText(historical04110);r=await call('/v1/reports',{method:'POST',body:historical04110});assert.equal(r.status,400,'historical 0.41.10 submissions are below the 0.80.3 floor');
let historical04111=fixture();historical04111.application.version='0.41.11';historical04111.application.versionCode=421;historical04111.reportText=reportText(historical04111);r=await call('/v1/reports',{method:'POST',body:historical04111});assert.equal(r.status,400,'historical 0.41.11 submissions are below the 0.80.3 floor');
...
let previousCurrent=fixture();previousCurrent.application.version='0.41.6';previousCurrent.application.versionCode=416;previousCurrent.reportText=reportText(previousCurrent);r=await call('/v1/reports',{method:'POST',body:previousCurrent});assert.equal(r.status,400,'0.41.6 submissions are below the 0.80.3 floor');
let previousStrict=fixture();previousStrict.application.version='0.41.4';previousStrict.application.versionCode=414;previousStrict.reportText=reportText(previousStrict);r=await call('/v1/reports',{method:'POST',body:previousStrict});assert.equal(r.status,400,'0.41.4 submissions are below the 0.80.3 floor');
let olderCurrent=fixture();olderCurrent.application.version='0.41.3';olderCurrent.application.versionCode=413;olderCurrent.reportText=reportText(olderCurrent);r=await call('/v1/reports',{method:'POST',body:olderCurrent});assert.equal(r.status,400,'0.41.3 submissions are below the 0.80.3 floor');
let previousProducer=fixture();previousProducer.application.version='0.41.2';previousProducer.application.versionCode=412;previousProducer.reportText=reportText(previousProducer);r=await call('/v1/reports',{method:'POST',body:previousProducer});assert.equal(r.status,400,'0.41.2 schema-compatible producer must remain accepted');
...
r=await call('/v1/reports',{method:'POST',body:goodSeparatedUnsupported});assert.equal(r.status,400,'historical 0.41.9 submissions are below the 0.80.3 floor');
...
r=await call('/v1/reports',{method:'POST',body:goodSeparatedUnavailable});assert.equal(r.status,400,'historical 0.41.9 submissions are below the 0.80.3 floor');
