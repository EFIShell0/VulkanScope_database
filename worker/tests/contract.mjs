...
let r=await call('/v1/health');assert.equal(r.status,200);let j=await r.json();assert.equal(j.normalizerVersion,16);assert.match(j.publishedVulkanSpec,/1\.4\.362/);assert.match(j.producerQueryBaseline,/0\.80\.10/);assert.match(j.compatibleProducer,/0\.80\.3\+/);
